...
/**
 * This class translates DRDA protocol from an application requester to JDBC for
 * Derby and then translates the results from Derby to DRDA for return to the
...
public class DRDAConnThread extends Thread {
	...
	private static int[] ACCRDB_REQUIRED = { CodePoint.RDBACCCL,
			CodePoint.CRRTKN, CodePoint.PRDID, CodePoint.TYPDEFNAM,
			CodePoint.TYPDEFOVR };
	...
	private boolean sendWarningsOnCNTQRY = false; // Send Warnings for SELECT if
													// true
	/** End this thread. */
	...
	// these fields are needed to feed back to jcc about a statement/procedure's
	// PKGNAMCSN
	/**
	 * The value returned by the previous call to <code>parsePKGNAMCSN()</code>.
	 */
	...
	// this flag is for an execute statement/procedure which actually returns a
	// result set;
	...
	// public key generated by Deffie-Hellman algorithm, to be passed to the
	// encrypter,
	...
	// Some byte[] constants that are frequently written into messages. It is
	// more efficient to
	// use these constants than to convert from a String each time
	// (This replaces the qryscraft_ and notQryscraft_ static exception
	// objects.)
	...
	private final static String AUTHENTICATION_PROVIDER_BUILTIN_CLASS = "org.apache.derby.impl.jdbc.authentication.BasicAuthenticationServiceImpl";

	private final static String AUTHENTICATION_PROVIDER_NONE_CLASS = "org.apache.derby.impl.jdbc.authentication.NoneAuthenticationServiceImpl";
	...
	private static final DRDAProtocolExceptionInfo dummy = new DRDAProtocolExceptionInfo(
			0, 0, 0, false);
	/**
	 * Tells if the reset / connect request is a deferred request. This
	 * information is used to work around a bug (DERBY-3596) in a compatible
	 * manner, which also avoids any changes in the client driver.
	 * <p>
	 ...
	 *
	 * @param session
	 *            Session requesting processing
	 * @param server
	 *            Server starting thread
	 * @param timeSlice
	 *            timeSlice for thread
	 * @param logConnections
	 **/

	public DRDAConnThread(Session session, NetworkServerControlImpl server,
			long timeSlice, boolean logConnections) {
	...
	/**
	 * Main routine for thread, loops until the thread is closed Gets a session,
	 * does work for the session
	 */
	@Override
	public void run() {
	 ...
	 */
	protected void markCommunicationsFailure(String arg1, String arg2,
			String arg3, String arg4) throws DRDAProtocolException {
	 ...
	 * 
	 * @param e
	 *            - Source exception that was thrown
	 * @param arg1
	 *            - info about the communications failure
	 * @param arg2
	 *            - info about the communications failure
	 ...
	 *                disconnect exception always thrown
	 */
	protected void markCommunicationsFailure(Exception e, String arg1,
			String arg2, String arg3, String arg4) throws DRDAProtocolException {
		if (e != null) {
			e.printStackTrace();
	...
	protected void throwSyntaxrm(int errcd, int cpArg)
		...
		throw new DRDAProtocolException(
				DRDAProtocolException.DRDA_Proto_SYNTAXRM, this, cpArg, errcd);
	}
	...
	protected void agentError(String msg) throws DRDAProtocolException {
		throw DRDAProtocolException.newAgentError(this,
				CodePoint.SVRCOD_PRMDMG, getDbName(), msg);
	}
	...
	protected static void println2Log(String dbname, String drdaID, String msg) {
		...
		logStream.printlnWithHeader("(DATABASE = " + dbname + "), (DRDAID = "
				+ drdaID + "), " + msg);
	}
	...
	protected void writeRDBNAM(String rdbnam) throws DRDAProtocolException {
		...
		int len = Math.max(CodePoint.RDBNAM_LEN,
	...
	private void initialize() {
		...
		/*
		 * At this stage we can initialize the strings as we have the
		 * CcsidManager for the DDMWriter.
		 */
	...
	/**
	 * In initial state for a session, determine whether this is a command
	 * session or a DRDA protocol session. A command session is for changing the
	 * configuration of the Net server, e.g., turning tracing on If it is a
	 * command session, process the command and close the session. If it is a
	 * DRDA session, exchange server attributes and change session state.
	 */
	private void sessionInitialState() throws Exception {
		// process NetworkServerControl commands - if it is not either valid
		// protocol let the
	...
	/**
	 * Cleans up and closes a result set if an exception is thrown when
	 * collecting QRYDTA in response to OPNQRY or CNTQRY.
	 *
	 * @param stmt
	 *            the DRDA statement to clean up
	 * @param sqle
	 *            the exception that was thrown
	 * @param writerMark
	 *            start index for the first DSS to clear from the output buffer
	 * @exception DRDAProtocolException
	 *                if a DRDA protocol error is detected
	 */
	private void cleanUpAndCloseResultSet(DRDAStatement stmt,
			SQLException sqle, int writerMark) throws DRDAProtocolException {
		if (stmt != null) {
	...
	private void processCommands() throws DRDAProtocolException {
		...
		do {
			...
			if (checkSecurityCodepoint) {
				verifyInOrderACCSEC_SECCHK(codePoint,
						session.getRequiredSecurityCodepoint());
			}
			...
			case CodePoint.CNTQRY:
				try {
					...
					if (stmt != null) {
						...
						checkWarning(null, null, stmt.getResultSet(), 0, false,
			...
			case CodePoint.EXCSQLIMM:
				try {
					...
					checkWarning(null, database.getDefaultStatement()
							.getStatement(), null, updateCount, true, true);
					writePBSD();
				} catch (SQLException e) {
					writer.clearDSSesBackToMark(writerMark);
					writeSQLCARDs(e, 0);
			...
			case CodePoint.PRPSQLSTT:
				...
				try {
					...
					if (sqldaType > 0) { // do write SQLDARD
						...
						checkWarning(database.getConnection(), null, null, 0,
								true, true);
					}
			...
			case CodePoint.OPNQRY:
				try {
					// activate the following to run the isValid(timeout) test
					// in jdbc4.ConnectionTest
					/*
					 * try { Thread.sleep(2000); } catch (InterruptedException
					 * ie) { System.out.println("interrupted exception: " +
					...
					if (pkgnamcsn != null) {
						...
						long currentVersion = ((EnginePreparedStatement) stmt.ps)
								.getVersionCounter();

						if (stmt.sqldaType == CodePoint.TYPSQLDA_LIGHT_OUTPUT
			...
			case CodePoint.RDBRLLBCK:
				try {
					...
					checkWarning(database.getConnection(), null, null, 0, true,
							true);
					// we only want to write one of these per transaction
					// so set to false in preparation for next command
					database.RDBUPDRM_sent = false;
			...
			case CodePoint.ACCSEC:
				...
				if (appRequester.supportsUtf8Ccsid()) {
					...
					/*
					 * This thread might serve several requests. Revert if not
					 * supported by current client.
					 */
					switchToEbcdic();
				}
			...
			case CodePoint.SECCHK:
				...
				break;
			/*
			 * since we don't support sqlj, we won't get bind commands from jcc,
			 * we might get it from ccc; just skip them.
			 */
			...
			case CodePoint.DSCSQLSTT:
				...
				try {
					...
					} catch (SQLException e2) { // should not get here since
												// doing nothing with ps
			...
			if (SanityManager.DEBUG) {
				...
				try {
					PiggyBackedSessionData pbsd = database
							.getPiggyBackedSessionData(false);
					// DERBY-3596
	 ...
	 * "terminate chain on error", we stop processing further commands in the
	 * chain nor do we send any reply for them. In accordance to this, a
	 * SQLERRRM message indicating the severe error must have been sent!
	 * (otherwise application requestor, such as JCC, would not terminate the
	 * receiving of chain replies.)
	 *
	 * Each DRDA command is processed independently. DRDA defines no
	 * interdependencies across chained commands. A command is processed the
	 * same when received within a set of chained commands or received
	 * separately. The chaining was originally defined as a way to save network
	 * costs.
	 *
	 * @param e
	...
	private void errorInChain(SQLException e) throws DRDAProtocolException {
		if (reader.terminateChainOnErr()
				&& (getExceptionSeverity(e) > CodePoint.SVRCOD_ERROR)) {
			if (SanityManager.DEBUG) {
	...
	private void exchangeServerAttributes() throws DRDAProtocolException {
		...
		if (SanityManager.DEBUG) {
			if (correlationID == 0) {
				SanityManager
						.THROWASSERT("Unexpected value for correlationId = "
								+ correlationID);
			}
		}
		...
		if (codePoint != CodePoint.EXCSAT) {
			// Throw PRCCNVRM
			throw new DRDAProtocolException(
					DRDAProtocolException.DRDA_Proto_PRCCNVRM, this, codePoint,
	...
	private boolean parseDRDAConnection() throws DRDAProtocolException {
		...
		// at this point if the security check failed, we're done, the session
		// failed
		...
		// If network server gets a null connection form InternalDriver, reply
		// with
		...
		if (databaseAccessException != null) {
			...
			return false;
		} else if (database.accessCount > 1) // already in conversation with
												// database
		{
	 ...
	 * Write RDB Failure
	 *
	 * Instance Variables SVRCOD - Severity Code - required RDBNAM - Relational
	 * Database name - required SRVDGN - Server Diagnostics - optional (not sent
	 * for now)
	 *
	 * @param codePoint
	...
	/*
	 * Check the database access exception and return the appropriate error
	 * codepoint. RDBNFNRM - Database not found RDBATHRM - Not Authorized
	...
	private int getRdbAccessErrorCodePoint() {
		...
		if (sqlState.regionMatches(0, SQLState.DATABASE_NOT_FOUND, 0, 5)) {
			...
			if (isAuthenticationException(databaseAccessException)
					|| sqlState.regionMatches(0,
							SQLState.AUTH_INVALID_USER_NAME, 0, 5)) {
	...
	/**
	 * There are multiple reasons for not getting a connection, and all these
	 * should throw SQLExceptions with SQL state 08004 according to the SQL
	 * standard. Since only one of these SQL states indicate that an
	 * authentication error has occurred, it is not enough to check that the SQL
	 * state is 08004 and conclude that authentication caused the exception to
	 * be thrown.
	 *
	 * This method tries to get a StandardException from the SQLException and
	 * use getMessageId on that object to check for authentication error instead
	 * of the SQL state we get from SQLExceptions#getSQLState. getMessageId
	 * returns the entire id as defined in SQLState (e.g. 08004.C.1), while
	 * getSQLState only return the 5 first characters (i.e. 08004 instead of
	 * 08004.C.1)
	 *
	 * If the SQLException isn't linked to a StandardException, the assumption
	 * that SQL State 08004 is caused by an authentication failure is followed
	 * even though this is not correct. This was the pre DERBY-3060 way of
	 * solving the issue.
	 *
	 * @param sqlException
	 *            The exception that is checked to see if this is really caused
	 *            by an authentication failure
	 * @return true if sqlException is (or has to be assumed to be) caused by an
	 *         authentication failure, false otherwise.
	 * @see SQLState
	 */
	 ...
	 *
	 * Username and password is verified by making a connection to the database
	 *
	...
	private int getConnFromDatabaseName() throws DRDAProtocolException {
		...
		databaseAccessException = null;
		// if we haven't got the correlation token yet, use session number for
		// drdaID
		...
		if ((database.securityMechanism == CodePoint.SECMEC_USRSSBPWD)
			...
			p.put(Attribute.DRDA_SECTKN_IN, DecryptionManager.toHexString(
					database.secTokenIn, 0, database.secTokenIn.length));
			p.put(Attribute.DRDA_SECTKN_OUT, DecryptionManager.toHexString(
					database.secTokenOut, 0, database.secTokenOut.length));
		}
		...
		try {
			...
			println2Log(database.getDatabaseName(), session.drdaID,
					"Driver not loaded" + e.getMessage());
			try {
				...
				try {
					...
					println2Log(
							...
							session.drdaID,
							"Failed attempt to reload driver "
									+ re.getMessage());
				}
				throw dpe;
			}
	...
	/**
	 * Parses EXCSAT (Exchange Server Attributes) Instance variables
	 * EXTNAM(External Name) - optional MGRLVLLS(Manager Levels) - optional
	 * SPVNAM(Supervisor Name) - optional SRVCLSNM(Server Class Name) - optional
	 * SRVNAM(Server Name) - optional, ignorable SRVRLSLV(Server Product Release
	 * Level) - optional, ignorable
	 *
	 * @exception DRDAProtocolException
	 */
	...
	/**
	 * Parses EXCSAT2 (Exchange Server Attributes) Instance variables
	 * EXTNAM(External Name) - optional MGRLVLLS(Manager Levels) - optional
	 * SPVNAM(Supervisor Name) - optional SRVCLSNM(Server Class Name) - optional
	 * SRVNAM(Server Name) - optional, ignorable SRVRLSLV(Server Product Release
	 * Level) - optional, ignorable
	 ...
	 * @exception DRDAProtocolException
	 * 
	 *                This parses a second occurrence of an EXCSAT command The
	 *                target must ignore the values for extnam, srvclsnm, srvnam
	 *                and srvrlslv. I am also going to ignore spvnam since it
	 *                should be null anyway. Only new managers can be added.
	 */
	...
	/**
	 * Parse manager levels Instance variables MGRLVL - repeatable, required
	 * CODEPOINT CCSIDMGR - CCSID Manager CMNAPPC - LU 6.2 Conversational
	 * Communications Manager CMNSYNCPT - SNA LU 6.2 SyncPoint Conversational
	 * Communications Manager CMNTCPIP - TCP/IP Communication Manager DICTIONARY
	 * - Dictionary RDB - Relational Database RSYNCMGR - Resynchronization
	 * Manager SECMGR - Security Manager SQLAM - SQL Application Manager
	 * SUPERVISOR - Supervisor SYNCPTMGR - Sync Point Manager VALUE
	 *
	...
	private void parseMGRLVLLS(int time) throws DRDAProtocolException {
		...
		if (errorManagers.size() > 0) {
			...
			throw new DRDAProtocolException(
					DRDAProtocolException.DRDA_Proto_MGRLVLRM, this, 0, 0, oa);
		}
	}

	/**
	 * Write reply to EXCSAT command Instance Variables EXTNAM - External Name
	 * (optional) MGRLVLLS - Manager Level List (optional) SRVCLSNM - Server
	 * Class Name (optional) - used by JCC SRVNAM - Server Name (optional)
	...
	/**
	 * Write manager levels The target server must not provide information for
	 * any target managers unless the source explicitly requests it. For each
	 * manager class, if the target server's support level is greater than or
	 * equal to the source server's level, then the source server's level is
	 * returned for that class if the target server can operate at the source's
	 * level; otherwise a level 0 is returned. If the target server's support
	 * level is less than the source server's level, the target server's level
	 * is returned for that class. If the target server does not recognize the
	 * code point of a manager class or does not support that class, it returns
	 * a level of 0. The target server then waits for the next command or for
	 * the source server to terminate communications. When the source server
	 * receives EXCSATRD, it must compare each of the entries in the mgrlvlls
	 * parameter it received to the corresponding entries in the mgrlvlls
	 * parameter it sent. If any level mismatches, the source server must decide
	 * whether it can use or adjust to the lower level of target support for
	 * that manager class. There are no architectural criteria for making this
	 * decision. The source server can terminate communications or continue at
	 * the target servers level of support. It can also attempt to use whatever
	 * commands its user requests while receiving error reply messages for real
	 * functional mismatches. The manager levels the source server specifies or
	 * the target server returns must be compatible with the manager-level
	 * dependencies of the specified manangers. Incompatible manager levels
	 * cannot be specified. Instance variables MGRLVL - repeatable, required
	 * CODEPOINT CCSIDMGR - CCSID Manager CMNAPPC - LU 6.2 Conversational
	 * Communications Manager CMNSYNCPT - SNA LU 6.2 SyncPoint Conversational
	 * Communications Manager CMNTCPIP - TCP/IP Communication Manager DICTIONARY
	 * - Dictionary RDB - Relational Database RSYNCMGR - Resynchronization
	 * Manager SECMGR - Security Manager SQLAM - SQL Application Manager
	 * SUPERVISOR - Supervisor SYNCPTMGR - Sync Point Manager XAMGR - XA manager
	 * VALUE
	 */
	 ...
	 *
	 * If the target server supports the SECMEC requested by the application
	 * requester then a single value is returned and it is identical to the
	 * SECMEC value in the ACCSEC command. If the target server does not support
	 * the SECMEC requested, then one or more values are returned and the
	 * application requester must choose one of these values for the security
	 * mechanism. We currently support - user id and password (default for JCC)
	 * - encrypted user id and password - strong password substitute (USRSSBPWD
	 * w/ Derby network client only)
	 *
	 * Instance variables SECMGRNM - security manager name - optional SECMEC -
	 * security mechanism - required RDBNAM - relational database name -
	 * optional SECTKN - security token - optional, (required if sec mech. needs
	 * it)
	 *
	...
	private int parseACCSEC() throws DRDAProtocolException {
		...
		while (codePoint != -1) {
			...
			case CodePoint.SECMEC:
				...
				if (SanityManager.DEBUG) {
					trace("parseACCSEC - Security mechanism = "
							+ securityMechanism);
				}
				...
				if ((server.getSecurityMechanism() != NetworkServerControlImpl.INVALID_OR_NOTSET_SECURITYMECHANISM)
					...
					if (securityMechanism != CodePoint.SECMEC_USRIDPWD
						...
						if (((securityMechanism != CodePoint.SECMEC_EUSRIDPWD) || (securityMechanism == CodePoint.SECMEC_EUSRIDPWD && !server
								.supportsEUSRIDPWD()))
						...
						// securityCheckCode = CodePoint.SECCHKCD_NOTSUPPORTED;
						{
							...
							try {
								...
								myPublicKey = decryptionManager
										.obtainPublicKey();
							} catch (SQLException e) {
								println2Log(null, session.drdaID,
										e.getMessage());
		...
		// need security token
		if (securityCheckCode == 0
				&& (database.securityMechanism == CodePoint.SECMEC_EUSRIDPWD || database.securityMechanism == CodePoint.SECMEC_USRSSBPWD)
		...
		if (securityCheckCode == 0
				&& (database.securityMechanism == CodePoint.SECMEC_USRIDPWD || database.securityMechanism == CodePoint.SECMEC_USRIDONL)
				&& database.secTokenIn != null) {
			securityCheckCode = CodePoint.SECCHKCD_SECTKNMISSING_OR_INVALID;
		}
		...
		session.setState((securityCheckCode == 0) ? Session.SECACC
				: Session.ATTEXC);
	...
	/**
	 * Parse OPNQRY Instance Variables RDBNAM - relational database name -
	 * optional PKGNAMCSN - RDB Package Name, Consistency Token and Section
	 * Number - required QRYBLKSZ - Query Block Size - required QRYBLKCTL -
	 * Query Block Protocol Control - optional MAXBLKEXT - Maximum Number of
	 * Extra Blocks - optional - default value 0 OUTOVROPT - Output Override
	 * Option QRYROWSET - Query Rowset Size - optional - level 7 MONITOR -
	...
	/**
	 * Parse OPNQRY objects Objects TYPDEFNAM - Data type definition name -
	 * optional TYPDEFOVR - Type defintion overrides - optional SQLDTA- SQL
	 * Program Variable Data - optional
	 *
	 * If TYPDEFNAM and TYPDEFOVR are supplied, they apply to the objects sent
	 * with the statement. Once the statement is over, the default values sent
	 * in the ACCRDB are once again in effect. If no values are supplied, the
	 * values sent in the ACCRDB are used. Objects may follow in one DSS or in
	 * several DSS chained together.
	 * 
	 * @throws DRDAProtocolException
	 * @throws SQLException
	 */
	...
	/**
	 * Parse QRYBLSZ - this gives the maximum size of the query blocks that can
	...
	/**
	 * Parse a QRYCLSIMP - Implicitly close non-scrollable cursor after end of
	 * data.
	 * 
	...
	/**
	 * Write a QRYPOPRM - Query Previously opened Instance Variables SVRCOD -
	 * Severity Code - required - 8 ERROR RDBNAM - Relational Database Name -
	 * required PKGNAMCSN - RDB Package Name, Consistency Token, and Section
	 * Number - required
	 * 
	 * @exception DRDAProtocolException
	 */
	...
	/**
	 * Write a QRYNOPRM - Query Not Opened Instance Variables SVRCOD - Severity
	 * Code - required - 4 Warning 8 ERROR RDBNAM - Relational Database Name -
	 * required PKGNAMCSN - RDB Package Name, Consistency Token, and Section
	 * Number - required
	 * 
	 * @param svrCod
	...
	/**
	 * Write a OPNQFLRM - Open Query Failure Instance Variables SVRCOD -
	 * Severity Code - required - 8 ERROR RDBNAM - Relational Database Name -
	 * required
	 *
	 * @param e
	...
	/**
	 * Write PKGNAMCSN Instance Variables NAMESYMDR - database name - not
	 * validated RDBCOLID - RDB Collection Identifier PKGID - RDB Package
	 * Identifier PKGCNSTKN - RDB Package Consistency Token PKGSN - RDB Package
	 * Section Number
	 *
	...
	private void writePKGNAMCSN(byte[] pkgcnstkn) throws DRDAProtocolException {
		...
		if (rdbnam.length() <= CodePoint.RDBNAM_LEN
				...
				&& pkgid.length() <= CodePoint.PKGID_LEN) { // if none of
															// RDBNAM, RDBCOLID
															// and PKGID have a
															// length of
															// more than 18, use
			...
			writer.writeScalarPaddedBytes(pkgcnstkn, CodePoint.PKGCNSTKN_LEN,
					(byte) 0);
	...
	/**
	 * Parse CNTQRY - Continue Query Instance Variables RDBNAM - Relational
	 * Database Name - optional PKGNAMCSN - RDB Package Name, Consistency Token,
	 * and Section Number - required QRYBLKSZ - Query Block Size - required
	 * QRYRELSCR - Query Relative Scrolling Action - optional QRYSCRORN - Query
	 * Scroll Orientation - optional - level 7 QRYROWNBR - Query Row Number -
	 * optional QRYROWSNS - Query Row Sensitivity - optional - level 7 QRYBLKRST
	 * - Query Block Reset - optional - level 7 QRYRTNDTA - Query Returns Data -
	 * optional - level 7 QRYROWSET - Query Rowset Size - optional - level 7
	 * QRYRFRTBL - Query Refresh Answer Set Table - optional NBRROW - Number of
	 * Fetch or Insert Rows - optional MAXBLKEXT - Maximum number of extra
	 * blocks - optional RTNEXTDTA - Return of EXTDTA Option - optional MONITOR
	 * - Monitor events - optional.
	 *
	 ...
	 * @throws SQLException
	 */
	private DRDAStatement parseCNTQRY() throws DRDAProtocolException,
			SQLException {
		byte val;
		Pkgnamcsn pkgnamcsn = null;
		boolean gotQryblksz = false;
		...
		stmt.setQueryOptions(blksize, qryrelscr, qryrownbr, qryrfrtbl, nbrrow,
				maxblkext, qryscrorn, qryrowsns, qryblkrst, qryrtndta,
				qryrowset, rtnextdta);
	 ...
	 *
	 * @param onlySkipSameIds
	 *            True if we _only_ want to skip DSS'es that are chained with
	 *            the SAME id as the current DSS. False means skip ALL chained
	 *            DSSes, whether they're chained with same or different ids.
	 * @exception DRDAProtocolException
	 */
	...
	/**
	 * Parse CNTQRY objects Instance Variables OUTOVR - Output Override
	 * Descriptor - optional
	 *
	 * @param stmt
	...
	/**
	 * Parse OUTOVR - Output Override Descriptor This specifies the output
	 * format for data to be returned as output to a SQL statement or as output
	 * from a query.
	 *
	 * @param stmt
	 ...
	 * @exception DRDAProtocolException
	 */
	private void parseOUTOVR(DRDAStatement stmt) throws DRDAProtocolException,
			SQLException {
		boolean first = true;
		int numVars;
		int dtaGrpLen;
		...
		while (true) {
			...
			if (first) {
				...
				System.arraycopy(oldoutovr_drdaType, 0, outovr_drdaType, 0,
						oldlen);
				start = oldlen;
			}
	...
	private void writePBSD() throws SQLException, DRDAProtocolException {
		...
		pbsd.setUnmodified();
		if (SanityManager.DEBUG) {
			PiggyBackedSessionData pbsdNew = database
					.getPiggyBackedSessionData(true);
			SanityManager.ASSERT(pbsdNew == pbsd,
					"pbsdNew and pbsd are expected to reference "
							+ "the same object");
			pbsd.refresh();
			SanityManager.ASSERT(!pbsd.isModified(), "pbsd=(" + pbsd
					+ ") is not expected to be modified");
		}
	}

	/**
	 * Write OPNQRYRM - Open Query Complete Instance Variables SVRCOD - Severity
	 * Code - required QRYPRCTYP - Query Protocol Type - required SQLCSRHLD -
	 * Hold Cursor Position - optional QRYATTSCR - Query Attribute for
	 * Scrollability - optional - level 7 QRYATTSNS - Query Attribute for
	 * Sensitivity - optional - level 7 QRYATTUPD - Query Attribute for
	 * Updatability -optional - level 7 QRYINSID - Query Instance Identifier -
	 * required - level 7 SRVDGN - Server Diagnostic Information - optional
	 *
	 * @param isDssObject
	 *            - return as a DSS object (part of a reply)
	 * @param stmt
	...
	private void writeOPNQRYRM(boolean isDssObject, DRDAStatement stmt)
		...
		// statement
		// in the stored procedure, not the holdability of the calling
		// statement.
		...
		if (sqlamLevel >= MGRLVL_7) {
			...
			// of the query. A query from an application would be nesting level
			// 0,
			// from a stored procedure, nesting level 1, from a recursive call
			// of
	...
	/**
	 * Write ABNUOWRM - query process has terminated in an error condition such
	 * as deadlock or lock timeout. Severity code is always error * @exception
	...
	/**
	 * Write ACCSECRD If the security mechanism is known, we just send it back
	 * along with the security token if encryption is going to be used. If the
	 * security mechanism is not known, we send a list of the ones we know.
	 * Instance Variables SECMEC - security mechanism - required SECTKN -
	 * security token - optional (required if security mechanism uses
	 * encryption) SECCHKCD - security check code - error occurred in processing
	 * ACCSEC
	 *
	...
	private void writeACCSECRD(int securityCheckCode)
		...
		if (securityCheckCode != CodePoint.SECCHKCD_NOTSUPPORTED) {
			writer.writeScalar2Bytes(CodePoint.SECMEC,
					database.securityMechanism);
		} else {
			// if server doesnt recognize or allow the client requested security
			// mechanism,
			// then need to return the list of security mechanisms
			// supported/allowed by the server

			// check if server is set to accept connections from client at a
			// certain
			// security mechanism, if so send only the security mechanism that
			// the
	...
	/**
	 * Parse security check Instance Variables SECMGRNM - security manager name
	 * - optional, ignorable SECMEC - security mechanism - required SECTKN -
	 * security token - optional, (required if encryption used) PASSWORD -
	 * password - optional, (required if security mechanism uses it) NEWPASSWORD
	 * - new password - optional, (required if sec mech. uses it) USRID - user
	 * id - optional, (required if sec mec. uses it) RDBNAM - database name -
	 * optional (required if databases can have own sec.)
	 *
	 * 
		...
		if (this.deferredReset) {
			...
			while (codePoint != -1) {
				...
				case CodePoint.SECMEC:
					...
					securityMechanism = reader.readNetworkShort();
					if (SanityManager.DEBUG) {
						trace("parseSECCHK - Security mechanism = "
								+ securityMechanism);
					}
				...
				case CodePoint.SECTKN:
					if ((database.securityMechanism != CodePoint.SECMEC_EUSRIDPWD)
						...
						securityCheckCode = CodePoint.SECCHKCD_SECTKNMISSING_OR_INVALID;
						reader.skipBytes();
					} else if (database.securityMechanism == CodePoint.SECMEC_EUSRIDPWD) {
						if (database.decryptedUserId == null) {
							try {
								database.decryptedUserId = reader
										.readEncryptedString(decryptionManager,
												database.securityMechanism,
												myPublicKey,
												database.secTokenIn);
							} catch (SQLException se) {
								println2Log(database.getDatabaseName(),
										session.drdaID, se.getMessage());
								if (securityCheckCode == 0) {
									// userid invalid
							...
							if (SanityManager.DEBUG) {
								trace("**decrypted userid is: "
										+ database.userId);
							}
						} else if (database.decryptedPassword == null) {
							try {
								database.decryptedPassword = reader
										.readEncryptedString(decryptionManager,
												database.securityMechanism,
												myPublicKey,
												database.secTokenIn);
						...
						if (database.passwordSubstitute == null) {
							...
							if (SanityManager.DEBUG) {
								trace("** Substitute Password is:"
										+ DecryptionManager
												.toHexString(
														database.passwordSubstitute,
														0,
														database.passwordSubstitute.length));
							}
							database.password = DecryptionManager.toHexString(
				...
				case CodePoint.RDBNAM:
					...
					if (database != null) {
						...
						// added code here in case we make the SECMEC session
						// rather
						// than database wide
						initializeDatabase(dbname);
					}
			...
			if (securityCheckCode == 0
				...
				// Note, we'll ignore encryptedUserId and encryptedPassword if
				// they
				// are also set
			}
			...
			// RESOLVE - when we do security we need to decrypt encrypted userid
			// & password
			// before proceeding
	...
	/**
	 * Write security check reply Instance variables SVRCOD - serverity code -
	 * required SECCHKCD - security check code - required SECTKN - security
	 * token - optional, ignorable SVCERRNO - security service error number
	...
	private void writeSECCHKRM(int securityCheckCode)
		...
		writer.writeScalar2Bytes(CodePoint.SVRCOD,
				svrcodFromSecchkcd(securityCheckCode));
		writer.writeScalar1Byte(CodePoint.SECCHKCD, securityCheckCode);
		writer.endDdmAndDss();
	...
	/**
	 * Parse access RDB Instance variables RDBACCCL - RDB Access Manager Class -
	 * required must be SQLAM CRRTKN - Correlation Token - required RDBNAM -
	 * Relational database name -required PRDID - Product specific identifier -
	 * required TYPDEFNAM - Data Type Definition Name -required TYPDEFOVR - Type
	 * definition overrides -required RDBALWUPD - RDB Allow Updates optional
	 * PRDDTA - Product Specific Data - optional - ignorable STTDECDEL -
	 * Statement Decimal Delimiter - optional STTSTRDEL - Statement String
	 * Delimiter - optional TRGDFTRT - Target Default Value Return - optional
	 *
	...
	private int parseACCRDB() throws DRDAProtocolException {
		...
		while (codePoint != -1) {
			...
			case CodePoint.CRRTKN:
				...
				long time = SignedBinary.getLong(database.crrtkn, l - 8,
						SignedBinary.BIG_ENDIAN); // as "long" as unique
				session.drdaID = reader.convertBytes(part1) + time + leftBrace
						+ session.connNum + rightBrace;
				if (SanityManager.DEBUG) {
					trace("******************************************drdaID is: "
							+ session.drdaID);
				}
		...
		if ((database.ccsidDBC != 0 && !server.supportsCCSID(database.ccsidDBC))
				|| (database.ccsidMBC != 0 && !server
						.supportsCCSID(database.ccsidMBC))) {
	 ...
	 *
	 * @param setDatabase
	 *            if true, set database' byte order, otherwise set statement's
	 * @param stmt
	 *            DRDAStatement, used when setDatabase is false
	 * @param typDefNam
	 *            TYPDEFNAM value
	 */
	private void setStmtOrDbByteOrder(boolean setDatabase, DRDAStatement stmt,
			String typDefNam) {
		int byteOrder = (typDefNam.equals(CodePoint.TYPDEFNAM_QTDSQLX86) ? SignedBinary.LITTLE_ENDIAN
				: SignedBinary.BIG_ENDIAN);
		if (setDatabase) {
	...
	/**
	 * Write Access to RDB Completed Instance Variables SVRCOD - severity code -
	 * 0 info, 4 warning -required PRDID - product specific identifier -required
	 * TYPDEFNAM - type definition name -required TYPDEFOVR - type definition
	 * overrides - required RDBINTTKN - token which can be used to interrupt DDM
	 * commands - optional CRRTKN - correlation token - only returned if we
	 * didn't get one from requester SRVDGN - server diagnostic information -
	 * optional PKGDFTCST - package default character subtype - optional USRID -
	 * User ID at the target system - optional SRVLST - Server List
	 * 
	 * @exception DRDAProtocolException
	 */
	 ...
	 * Variables CCSIDSBC - CCSID for Single-Byte - optional CCSIDDBC - CCSID
	 * for Double-Byte - optional CCSIDMBC - CCSID for Mixed-byte characters
	 * -optional
	 *
	 * @param st
	...
	private void parseTYPDEFOVR(DRDAStatement st) throws DRDAProtocolException {
		...
		while (codePoint != -1) {
			...
			case CodePoint.CCSIDSBC:
				...
				try {
					ccsidSBCEncoding = CharacterEncodings
							.getJavaEncoding(ccsidSBC);
				} catch (Exception e) {
					valueNotSupported(CodePoint.CCSIDSBC);
				}
				if (SanityManager.DEBUG) {
					trace("ccsidsbc = " + ccsidSBC + " encoding = "
							+ ccsidSBCEncoding);
				}
				break;
			case CodePoint.CCSIDDBC:
				...
				try {
					ccsidDBCEncoding = CharacterEncodings
							.getJavaEncoding(ccsidDBC);
				} catch (Exception e) {
				...
				if (SanityManager.DEBUG) {
					trace("ccsiddbc = " + ccsidDBC + " encoding = "
							+ ccsidDBCEncoding);
				}
				break;
			case CodePoint.CCSIDMBC:
				...
				try {
					ccsidMBCEncoding = CharacterEncodings
							.getJavaEncoding(ccsidMBC);
				} catch (Exception e) {
					// we write a warning later for this so no error
				...
				if (SanityManager.DEBUG) {
					trace("ccsidmbc = " + ccsidMBC + " encoding = "
							+ ccsidMBCEncoding);
				}
				break;
			default:
	...
	/**
	 * Parse PRPSQLSTT - Prepare SQL Statement Instance Variables RDBNAM -
	 * Relational Database Name - optional PKGNAMCSN - RDB Package Name,
	 * Consistency Token, and Section Number - required RTNSQLDA - Return SQL
	 * Descriptor Area - optional MONITOR - Monitor events - optional.
	 * 
	 * @return return 0 - don't return sqlda, 1 - return input sqlda, 2 - return
	 *         output sqlda
	 * @throws DRDAProtocolException
	 * @throws SQLException
	 */
	private int parsePRPSQLSTT() throws DRDAProtocolException, SQLException {
		...
		//TODO: el statement no es el q toca
	...
	/**
	 * Parse PRPSQLSTT objects Objects TYPDEFNAM - Data type definition name -
	 * optional TYPDEFOVR - Type defintion overrides - optional SQLSTT - SQL
	 * Statement required SQLATTR - Cursor attributes on prepare - optional -
	 * level 7
	 *
	 * If TYPDEFNAM and TYPDEFOVR are supplied, they apply to the objects sent
	 * with the statement. Once the statement is over, the default values sent
	 ...
	 * Can have combination of following, eg INSENSITIVE SCROLL WITH HOLD
	 * Possible strings are SENSITIVE DYNAMIC SCROLL [FOR UPDATE] SENSITIVE
	 * STATIC SCROLL [FOR UPDATE] INSENSITIVE SCROLL FOR UPDATE WITH HOLD
	 *
	 * @param stmt
	 *            DRDAStatement
	 * @exception DRDAProtocolException
	...
	protected void parseSQLATTR(DRDAStatement stmt)
		...
		if (attrs.indexOf("INSENSITIVE SCROLL") != -1
				|| attrs.indexOf("SCROLL INSENSITIVE") != -1) // CLI
		{
			stmt.scrollType = ResultSet.TYPE_SCROLL_INSENSITIVE;
			stmt.concurType = ResultSet.CONCUR_READ_ONLY;
			validAttribute = true;
		}
		if ((attrs.indexOf("SENSITIVE DYNAMIC SCROLL") != -1)
				|| (attrs.indexOf("SENSITIVE STATIC SCROLL") != -1)) {
	...
	/**
	 * Parse DSCSQLSTT - Describe SQL Statement previously prepared Instance
	 * Variables TYPSQLDA - sqlda type expected (output or input) RDBNAM -
	 * relational database name - optional PKGNAMCSN - RDB Package Name,
	 * Consistency Token and Section Number - required MONITOR - Monitor events
	 * - optional.
	 *
	 ...
	 * Instance Variables RDBNAM - relational database name - optional PKGNAMCSN
	 * - RDB Package Name, Consistency Token and Section Number - required
	 * OUTEXP - Output expected NBRROW - Number of rows to be inserted if it's
	 * an insert PRCNAM - procedure name if specified by host variable, not
	 * needed for Derby QRYBLKSZ - query block size MAXRSLCNT - max resultset
	 * count MAXBLKEXT - Max number of extra blocks RSLSETFLG - resultset flag
	 * RDBCMTOK - RDB Commit Allowed - optional OUTOVROPT - output override
	 * option QRYROWSET - Query Rowset Size - Level 7 MONITOR - Monitor events -
	 * optional.
	 *
	 * @throws DRDAProtocolException
	 * @throws SQLException
	...
	private void parseEXCSQLSTT() throws DRDAProtocolException, SQLException {
		...
		stmt = database.getDRDAStatement(pkgnamcsn);
		boolean isProcedure = (procName != null || (stmt != null
				&& stmt.wasExplicitlyPrepared() && stmt.isCall));
		...
		if (pendingStatementTimeout >= 0) {
			stmt.getPreparedStatement()
					.setQueryTimeout(pendingStatementTimeout);
			pendingStatementTimeout = -1;
		}
		...
		do {
			if (hasResultSet) {
				...
				/*
				 * Currently, if LMTBLKPRC is used, a pre-condition is that no
				 * lob columns. But in the future, when we do support LOB in
				 * LMTBLKPRC, the drda spec still does not allow LOB to be sent
				 * with OPNQRYRM. So this "if" here will have to add
				 * "no lob columns".
				 */
				if (stmt.getQryprctyp() == CodePoint.LMTBLKPRC) {
					writeQRYDTA(stmt);
				...
				checkWarning(database.getConnection(), stmt.ps, null,
	 ...
	 * to be executed as part of the command Since we don't have a SQL commit or
	 * rollback command, we will just ignore this for now
	 *
	 * @exception DRDAProtocolException
	 */
	...
	/**
	 * Parse EXCSQLSTT command objects Command Objects TYPDEFNAM - Data Type
	 * Definition Name - optional TYPDEFOVR - TYPDEF Overrides -optional SQLDTA
	 * - optional, variable data, specified if prpared statement has input
	 * parameters EXTDTA - optional, externalized FD:OCA data OUTOVR - output
	 * override descriptor, not allowed for stored procedure calls
	 *
	 * If TYPDEFNAM and TYPDEFOVR are supplied, they apply to the objects sent
	 * with the statement. Once the statement is over, the default values sent
	 * in the ACCRDB are once again in effect. If no values are supplied, the
	...
	private void writeSQLRSLRD(DRDAStatement stmt)
		...
		for (int i = 0; i < numResults; i++) {
			...
			writer.writeInt(1); // num of rows XXX resolve, it doesn't matter
								// for now
	...
	/**
	 * Write RSLSETRM Instance variables SVRCOD - Severity code - Information
	 * only - required PKGSNLST - list of PKGNAMCSN -required SRVDGN - Server
	...
	private void parseSQLDTA(DRDAStatement stmt) throws DRDAProtocolException,
			SQLException {
		try {
	...
	/**
	 * Parse SQLDTA - SQL program variable data Instance Variables FDODSC -
	 * FD:OCA data descriptor - required FDODTA - FD:OCA data - optional
	 * 
	 * @throws DRDAProtocolException
	 * @throws SQLException
	 */
	private void parseSQLDTA_work(DRDAStatement stmt)
		...
		while (codePoint != -1) {
			...
			case CodePoint.FDODSC:
				...
				{
					StringBuilder marks = new StringBuilder(); // construct
																// parameter
																// marks
					marks.append("(?");
	...
	private int getByteOrder() {
		...
		return ((stmt != null && stmt.typDefNam != null) ? stmt.byteOrder
				: database.byteOrder);
	}
	 ...
	 * consistently read and write date and time values using the same calendar.
	 * Since the local default calendar may not be able to represent all times
	 * (for instance because the time would fall into a non-existing hour of the
	 * day when switching to daylight saving time, see DERBY-4582), we use the
	 * GMT time zone which doesn't observe daylight saving time.
	 *
	...
	private void readAndSetParams(int i, DRDAStatement stmt,
			ParameterMetaData pmeta) throws DRDAProtocolException, SQLException {
		PreparedStatement ps = stmt.getPreparedStatement();
		...
		case DRDAConstants.DRDA_TYPE_NDATE: {
			String paramVal = reader.readStringData(10).trim(); // parameter may
																// be char value
			if (SanityManager.DEBUG) {
		...
		case DRDAConstants.DRDA_TYPE_NTIME: {
			String paramVal = reader.readStringData(8).trim(); // parameter may
																// be char value
			if (SanityManager.DEBUG) {
		...
		case DRDAConstants.DRDA_TYPE_NTIMESTAMP: {
			// JCC represents ts in a slightly different format than Java
			// standard, so
			// we do the conversion to Java standard here.
			int timestampLength = appRequester.getTimestampLength();

			String paramVal = reader.readStringData(timestampLength).trim(); // parameter
																				// may
																				// be
																				// char
																				// value
		...
		case DRDAConstants.DRDA_TYPE_NLONGVARBYTE: {
			int length = reader.readNetworkShort(); // protocol control data
													// always follows big endian
			if (SanityManager.DEBUG) {
		...
		case DRDAConstants.DRDA_TYPE_NLOBCDBCS: {
			...
			if (length != 0) // can be -1 for CLI if "data at exec" mode, see
								// clifp/exec test
			{
		...
		case DRDAConstants.DRDA_TYPE_NLOBLOC: {
			...
			// Hash map.
			java.sql.Blob blobFromLocator = (java.sql.Blob) database
					.getConnection().getLOBMapping(paramVal);
		...
		case DRDAConstants.DRDA_TYPE_NCLOBLOC: {
			...
			// Hash Map.
			java.sql.Clob clobFromLocator = (java.sql.Clob) database
					.getConnection().getLOBMapping(paramVal);
	...
	private Object readUDT() throws DRDAProtocolException {
		int length = reader.readNetworkShort(); // protocol control data always
												// follows big endian
		if (SanityManager.DEBUG) {
			trace("===== udt param length is: " + length);
		}
		...
		try {
			...
			markCommunicationsFailure(e, "DRDAConnThread.readUDT()", "",
					e.getMessage(), "*");
			return null;
		}
	}
	...
	private long readLobLength(int extLenIndicator)
		...
		default:
			throwSyntaxrm(CodePoint.SYNERRCD_INCORRECT_EXTENDED_LEN,
					extLenIndicator);
			return 0L;
		}
	 ...
	 * @throws IllegalArgumentException
	 *             if the timestamp is not correctly formatted
	 */
	...
	private void readAndSetAllExtParams(final DRDAStatement stmt,
			final boolean streamLOB) throws SQLException, DRDAProtocolException {
		...
		for (int i = 0; i < numExt; i++) {
			...
			readAndSetExtParam(paramPos, stmt,
					stmt.getParamDRDAType(paramPos + 1),
					stmt.getParamLen(paramPos + 1), doStreamLOB);
	 ...
	 * @throws SQLException
	 */
	private void readAndSetExtParam(int i, DRDAStatement stmt, int drdaType,
			int extLen, boolean streamLOB) throws DRDAProtocolException,
			SQLException {
		...
		final EXTDTAReaderInputStream stream = reader
				.getEXTDTAReaderInputStream(checkNullability);
		...
		try {
			...
			case DRDAConstants.DRDA_TYPE_LOBCMIXED:
			case DRDAConstants.DRDA_TYPE_NLOBCMIXED:
				setAsCharacterStream(stmt, i + 1, stream, streamLOB, encoding);
				break;
			default:
	...
	/**
	 * Parse EXCSQLIMM - Execute Immediate Statement Instance Variables RDBNAM -
	 * relational database name - optional PKGNAMCSN - RDB Package Name,
	 * Consistency Token and Section Number - required RDBCMTOK - RDB Commit
	 * Allowed - optional MONITOR - Monitor Events - optional
	 *
	 * Command Objects TYPDEFNAM - Data Type Definition Name - optional
	 * TYPDEFOVR - TYPDEF Overrides -optional SQLSTT - SQL Statement -required
	 *
	 * @return update count
	 * @throws DRDAProtocolException
	 * @throws SQLException
	...
	/**
	 * Parse EXCSQLSET - Execute Set SQL Environment Instance Variables RDBNAM -
	 * relational database name - optional PKGNAMCT - RDB Package Name,
	 * Consistency Token - optional MONITOR - Monitor Events - optional
	 *
	 * Command Objects TYPDEFNAM - Data Type Definition Name - required
	 * TYPDEFOVR - TYPDEF Overrides - required SQLSTT - SQL Statement - required
	 * (at least one; may be more)
	 *
	 * @throws DRDAProtocolException
	 * @throws SQLException
	 */
	...
	/**
	 * Parse EXCSQLIMM objects Objects TYPDEFNAM - Data type definition name -
	 * optional TYPDEFOVR - Type defintion overrides SQLSTT - SQL Statement
	 * required
	 *
	 * If TYPDEFNAM and TYPDEFOVR are supplied, they apply to the objects sent
	 * with the statement. Once the statement is over, the default values sent
	 ...
	 * @throws SQLException
	 */
	private String parseEXECSQLIMMobjects() throws DRDAProtocolException,
			SQLException {
		String sqlStmt = null;
		int codePoint;
	...
	public void execSQLSTT(String sqlStmt, DRDAStatement drdaStmt)
			throws SQLException {
		drdaStmt.getStatement().executeUpdate(sqlStmt);
	}

	/**
	 * Parse EXCSQLSET objects Objects TYPDEFNAM - Data type definition name -
	 * optional TYPDEFOVR - Type defintion overrides - optional SQLSTT - SQL
	 * Statement - required (a list of at least one)
	 *
	 ...
	 * @throws SQLException
	 */
	private void parseEXCSQLSETobjects() throws DRDAProtocolException,
			SQLException {
			...
			while (reader.moreDssData()) {
				...
				case CodePoint.SQLSTT:
					...
					if (sqlStmt.startsWith(TIMEOUT_STATEMENT)) {
						String timeoutString = sqlStmt
								.substring(TIMEOUT_STATEMENT.length());
						pendingStatementTimeout = Integer
	...
	/**
	 * Write RDBUPDRM Instance variables SVRCOD - Severity code - Information
	 * only - required RDBNAM - Relational database name -required SRVDGN -
	 ...
	 * Instance Variables NAMESYMDR - database name - not validated RDBCOLID -
	 * RDB Collection Identifier PKGID - RDB Package Identifier PKGCNSTKN - RDB
	 * Package Consistency Token PKGSN - RDB Package Section Number
	 *
	...
	private Pkgnamcsn parsePKGNAMCSN() throws DRDAProtocolException {
		...
		} else // extended format
		{
			...
			if (length < CodePoint.RDBNAM_LEN
					|| length > CodePoint.RDBNAM_MAX_NAME) {
				badObjectLength(CodePoint.RDBNAM);
			}
		...
		if ((prevPkgnamcsn == null) || rdbnam.wasModified()
				|| rdbcolid.wasModified() || pkgid.wasModified()
			...
			prevPkgnamcsn = new Pkgnamcsn(rdbnam.toString(),
					rdbcolid.toString(), pkgid.toString(), pkgsn,
	 ...
	 * Format I2 - VCM Length N bytes - VCM value I2 - VCS Length N bytes - VCS
	 * value Only 1 of VCM length or VCS length can be non-zero
	 *
	 * @return string value
	 */
	 ...
	 * Format 1 byte - null indicator I4 - mixed character length N bytes -
	 * mixed character string 1 byte - null indicator I4 - single character
	 * length N bytes - single character length string
	 *
	 * @return string value
	 * @exception DRDAProtocolException
	 */
	...
	/**
	 * Parse CLSQRY Instance Variables RDBNAM - relational database name -
	 * optional PKGNAMCSN - RDB Package Name, Consistency Token and Section
	 * Number - required QRYINSID - Query Instance Identifier - required - level
	 * 7 MONITOR - Monitor events - optional.
	 *
	 * @return DRDAstatement being closed
	 * @throws DRDAProtocolException
	 * @throws SQLException
	 */
	private DRDAStatement parseCLSQRY() throws DRDAProtocolException,
			SQLException {
		Pkgnamcsn pkgnamcsn = null;
		reader.markCollection();
		long qryinsid = 0;
	...
	/**
	 * Parse MONITOR DRDA spec says this is optional. Since we don't currently
	 * support it, we just ignore.
	 */
	...
	private void writeSQLCARDs(SQLException e, long updateCount,
			boolean sendSQLERRRM) throws DRDAProtocolException {
		...
		// instead of writing a chain of sql error or warning, we send the first
		// one, this is
	 ...
	 *
	 * @param e
	 *            the exception or warning to get the SQLCODE for
	...
	private int getSqlCode(SQLException e) {
		...
		int severity = Math.max(ExceptionSeverity.NO_APPLICABLE_SEVERITY,
				e.getErrorCode());
		return -severity - 1;
	}

	private void writeSQLCARD(SQLException e, long updateCount, long rowCount)
			throws DRDAProtocolException {
		writer.createDssObject();
		writer.startDdm(CodePoint.SQLCARD);
		...
		if (e != null) {
			...
			if (sqlState.regionMatches(0, SQLState.CLOUDSCAPE_SYSTEM_SHUTDOWN,
					0, 5)) {
	 ...
	 * Write SQLERRRM
	 *
	 * Instance Variables SVRCOD - Severity Code - required
	 *
	 * @param severity
	 *            severity of error
	 *
	 ...
	 * Write SQLCAGRP
	 *
	 * SQLCAGRP : FDOCA EARLY GROUP SQL Communcations Area Group Description
	 *
	 * FORMAT FOR SQLAM <= 6 SQLCODE; DRDA TYPE I4; ENVLID 0x02; Length Override
	 * 4 SQLSTATE; DRDA TYPE FCS; ENVLID 0x30; Length Override 5 SQLERRPROC;
	 * DRDA TYPE FCS; ENVLID 0x30; Length Override 8 SQLCAXGRP; DRDA TYPE N-GDA;
	 * ENVLID 0x52; Length Override 0
	 *
	 * FORMAT FOR SQLAM >= 7 SQLCODE; DRDA TYPE I4; ENVLID 0x02; Length Override
	 * 4 SQLSTATE; DRDA TYPE FCS; ENVLID 0x30; Length Override 5 SQLERRPROC;
	 * DRDA TYPE FCS; ENVLID 0x30; Length Override 8 SQLCAXGRP; DRDA TYPE N-GDA;
	 * ENVLID 0x52; Length Override 0 SQLDIAGGRP; DRDA TYPE N-GDA; ENVLID 0x56;
	 * Length Override 0
	 *
	 * @param e
	...
	private void writeSQLCAGRP(SQLException e, long updateCount, long rowCount)
		...
		// SQLCAXGRP
		writeSQLCAXGRP(updateCount, rowCount, buildSqlerrmc(e),
				e.getNextException());
	}

	/**
	 * Same as writeSQLCAGRP, but optimized for the case when there is no real
	 * exception, i.e. the exception is null, or "End of data"
	 *
	 * SQLCAGRP : FDOCA EARLY GROUP SQL Communcations Area Group Description
	 *
	 * FORMAT FOR SQLAM <= 6 SQLCODE; DRDA TYPE I4; ENVLID 0x02; Length Override
	 ...
	 *
	 * @param sqlState
	 *            SQLState (already converted to UTF8)
	 * @param sqlcode
	 *            sqlcode
	 * @param updateCount
	 * @param rowCount
	...
	private void writeSQLCAGRP(byte[] sqlState, int sqlcode, long updateCount,
	 ...
	 */
	private static String SQLERRMC_TOKEN_DELIMITER = new String(
			new char[] { (char) 20 });
	...
	/**
	 * Create error message or message argements to return to client. The
	 * SQLERRMC will normally be passed back to the server in a call to the
	 * SYSIBM.SQLCAMESSAGE but for severe exceptions the stored procedure call
	 * cannot be made. So for Severe messages we will just send the message
	 * text.
	 * 
	 * This method will also truncate the value according the client capacity.
	 * CCC can only handle 70 characters.
	 * 
	 * Server sends the sqlerrmc using UTF8 encoding to the client. To get the
	 * message, client sends back information to the server calling
	 * SYSIBM.SQLCAMESSAGE (see Sqlca.getMessage). Several parameters are sent
	 * to this procedure including the locale, the sqlerrmc that the client
	 * received from the server. On server side, the procedure SQLCAMESSAGE in
	 * SystemProcedures then calls the MessageService.getLocalizedMessage to
	 * retrieve the localized error message. In
	 * MessageService.getLocalizedMessage the sqlerrmc that is passed in, is
	 * parsed to retrieve the message id. The value it uses to parse the
	 * MessageId is char value of 20, otherwise it uses the entire sqlerrmc as
	 * the message id. This messageId is then used to retrieve the localized
	 * message if present, to the client.
	 * 
	 * @param se
	...
	private String buildSqlerrmc(SQLException se) {
		...
		if (se instanceof DataTruncation) {
			...
			// we have no hope of succussfully calling the SYSIBM.SQLCAMESSAGE
			// send
	...
	/**
	 * Build preformatted SQLException text for severe exceptions or
	 * SQLExceptions that are not Derby exceptions. Just send the message text
	 * localized to the server locale.
	 * 
	 * @param se
	 *            SQLException for which to build SQLERRMC
	 * @return preformated message text with messages separted by
	 *         SQLERRMC_PREFORMATED_MESSAGE_DELIMITER
	 * 
	 */
	...
	/**
	 * Build Tokenized SQLERRMC to just send the tokenized arguments to the
	 * client. for a Derby SQLException or an SQLException thrown by user code.
	...
	private String buildTokenizedSqlerrmc(SQLException se) {
		...
		do {
			...
			if (se != null) {
				sqlerrmc += SystemProcedures.SQLERRMC_MESSAGE_DELIMITER
						+ se.getSQLState() + ":";
			}
		} while (se != null);
		return sqlerrmc;
	 ...
	 *
	 * @param dt
	 *            the {@code DataTruncation} instance to serialize
	 * @return the SQLERRMC string with all fields of the warning
	 */
	private String buildDataTruncationSqlerrmc(DataTruncation dt) {
		return dt.getIndex() + SQLERRMC_TOKEN_DELIMITER + dt.getParameter()
				+ SQLERRMC_TOKEN_DELIMITER + dt.getRead()
				+ SQLERRMC_TOKEN_DELIMITER + dt.getDataSize()
				+ SQLERRMC_TOKEN_DELIMITER + dt.getTransferSize();
	}

	/**
	 * Write SQLCAXGRP
	 *
	 * SQLCAXGRP : EARLY FDOCA GROUP SQL Communications Area Exceptions Group
	 * Description
	 *
	 * FORMAT FOR SQLAM <= 6 SQLRDBNME; DRDA TYPE FCS; ENVLID 0x30; Length
	 * Override 18 SQLERRD1; DRDA TYPE I4; ENVLID 0x02; Length Override 4
	 * SQLERRD2; DRDA TYPE I4; ENVLID 0x02; Length Override 4 SQLERRD3; DRDA
	 * TYPE I4; ENVLID 0x02; Length Override 4 SQLERRD4; DRDA TYPE I4; ENVLID
	 * 0x02; Length Override 4 SQLERRD5; DRDA TYPE I4; ENVLID 0x02; Length
	 * Override 4 SQLERRD6; DRDA TYPE I4; ENVLID 0x02; Length Override 4
	 * SQLWARN0; DRDA TYPE FCS; ENVLID 0x30; Length Override 1 SQLWARN1; DRDA
	 * TYPE FCS; ENVLID 0x30; Length Override 1 SQLWARN2; DRDA TYPE FCS; ENVLID
	 * 0x30; Length Override 1 SQLWARN3; DRDA TYPE FCS; ENVLID 0x30; Length
	 * Override 1 SQLWARN4; DRDA TYPE FCS; ENVLID 0x30; Length Override 1
	 * SQLWARN5; DRDA TYPE FCS; ENVLID 0x30; Length Override 1 SQLWARN6; DRDA
	 * TYPE FCS; ENVLID 0x30; Length Override 1 SQLWARN7; DRDA TYPE FCS; ENVLID
	 * 0x30; Length Override 1 SQLWARN8; DRDA TYPE FCS; ENVLID 0x30; Length
	 * Override 1 SQLWARN9; DRDA TYPE FCS; ENVLID 0x30; Length Override 1
	 * SQLWARNA; DRDA TYPE FCS; ENVLID 0x30; Length Override 1 SQLERRMSG_m; DRDA
	 * TYPE VCM; ENVLID 0x3E; Length Override 70 SQLERRMSG_s; DRDA TYPE VCS;
	 * ENVLID 0x32; Length Override 70
	 *
	 * FORMAT FOR SQLAM >= 7 SQLERRD1; DRDA TYPE I4; ENVLID 0x02; Length
	 * Override 4 SQLERRD2; DRDA TYPE I4; ENVLID 0x02; Length Override 4
	 * SQLERRD3; DRDA TYPE I4; ENVLID 0x02; Length Override 4 SQLERRD4; DRDA
	 * TYPE I4; ENVLID 0x02; Length Override 4 SQLERRD5; DRDA TYPE I4; ENVLID
	 * 0x02; Length Override 4 SQLERRD6; DRDA TYPE I4; ENVLID 0x02; Length
	 * Override 4 SQLWARN0; DRDA TYPE FCS; ENVLID 0x30; Length Override 1
	 * SQLWARN1; DRDA TYPE FCS; ENVLID 0x30; Length Override 1 SQLWARN2; DRDA
	 * TYPE FCS; ENVLID 0x30; Length Override 1 SQLWARN3; DRDA TYPE FCS; ENVLID
	 * 0x30; Length Override 1 SQLWARN4; DRDA TYPE FCS; ENVLID 0x30; Length
	 * Override 1 SQLWARN5; DRDA TYPE FCS; ENVLID 0x30; Length Override 1
	 * SQLWARN6; DRDA TYPE FCS; ENVLID 0x30; Length Override 1 SQLWARN7; DRDA
	 * TYPE FCS; ENVLID 0x30; Length Override 1 SQLWARN8; DRDA TYPE FCS; ENVLID
	 * 0x30; Length Override 1 SQLWARN9; DRDA TYPE FCS; ENVLID 0x30; Length
	 * Override 1 SQLWARNA; DRDA TYPE FCS; ENVLID 0x30; Length Override 1
	 * SQLRDBNAME; DRDA TYPE VCS; ENVLID 0x32; Length Override 1024 SQLERRMSG_m;
	 * DRDA TYPE VCM; ENVLID 0x3E; Length Override 70 SQLERRMSG_s; DRDA TYPE
	 * VCS; ENVLID 0x32; Length Override 70
	 * 
	 * @param nextException
	 *            SQLException encountered
	 * @param sqlerrmc
	 ...
	 * @exception DRDAProtocolException
	 */
	private void writeSQLCAXGRP(long updateCount, long rowCount,
			String sqlerrmc, SQLException nextException)
			throws DRDAProtocolException {
	 ...
	 * Nullable Group SQLDIAGSTT; DRDA TYPE N-GDA; ENVLID 0xD3; Length Override
	 * 0 SQLDIAGCN; DRFA TYPE N-RLO; ENVLID 0xF6; Length Override 0 SQLDIAGCI;
	 * DRDA TYPE N-RLO; ENVLID 0xF5; Length Override 0
	 */
	private void writeSQLDIAGGRP(SQLException nextException)
		...
		while (!(nextException == null
				|| SQLState.ROW_UPDATED.equals(nextException.getSQLState()) || SQLState.ROW_DELETED
					.equals(nextException.getSQLState()))) {
			nextException = nextException.getNextException();
		}

		if ((nextException == null) || (diagnosticLevel == CodePoint.DIAGLVL0)) {
			writer.writeByte(CodePoint.NULLDATA);
			return;
		}
	...
	/**
	 * writeSQLDIAGCI: SQL Diagnostics Condition Information Array - Identity
	 * 0xF5 SQLNUMROW; ROW LID 0x68; ELEMENT TAKEN 0(all); REP FACTOR 1
	...
	private void writeSQLDIAGCI(SQLException nextException)
		...
		while (se != null) {
			...
			if (diagnosticLevel == CodePoint.DIAGLVL0) {
				...
				StandardException ferry = StandardException
						.getArgumentFerry(se);
				if (ferry != null) {
					Object[] args = ferry.getArguments();
					for (int i = 0; args != null && i < args.length; i++) {
	...
	/**
	 * writeSQLNUMROW: Writes SQLNUMROW : FDOCA EARLY ROW SQL Number of Elements
	 * Row Description FORMAT FOR SQLAM LEVELS SQLNUMGRP; GROUP LID 0x58;
	 * ELEMENT TAKEN 0(all); REP FACTOR 1
	 */
	...
	/**
	 * writeSQLNUMGRP: Writes SQLNUMGRP : FDOCA EARLY GROUP SQL Number of
	 * Elements Group Description FORMAT FOR ALL SQLAM LEVELS SQLNUM; DRDA TYPE
	 * I2; ENVLID 0x04; Length Override 2
	 */
	...
	/**
	 * writeSQLDCROW: SQL Diagnostics Condition Row - Identity 0xE5 SQLDCGRP;
	 * GROUP LID 0xD5; ELEMENT TAKEN 0(all); REP FACTOR 1
	 */
	private void writeSQLDCROW(long rowNum, int sqlCode, String sqlState,
			String dbname, String sqlerrmc) throws DRDAProtocolException {
	 ...
	 * 
	 * SQLDCCODE; DRDA TYPE I4; ENVLID 0x02; Length Override 4 SQLDCSTATE; DRDA
	 * TYPE FCS; ENVLID Ox30; Lengeh Override 5 SQLDCREASON; DRDA TYPE I4;
	 * ENVLID 0x02; Length Override 4 SQLDCLINEN; DRDA TYPE I4; ENVLID 0x02;
	 * Length Override 4 SQLDCROWN; DRDA TYPE FD; ENVLID 0x0E; Lengeh Override
	 * 31 SQLDCER01; DRDA TYPE I4; ENVLID 0x02; Length Override 4 SQLDCER02;
	 * DRDA TYPE I4; ENVLID 0x02; Length Override 4 SQLDCER03; DRDA TYPE I4;
	 * ENVLID 0x02; Length Override 4 SQLDCER04; DRDA TYPE I4; ENVLID 0x02;
	 * Length Override 4 SQLDCPART; DRDA TYPE I4; ENVLID 0x02; Length Override 4
	 * SQLDCPPOP; DRDA TYPE I4; ENVLID 0x02; Length Override 4 SQLDCMSGID; DRDA
	 * TYPE FCS; ENVLID 0x30; Length Override 10 SQLDCMDE; DRDA TYPE FCS; ENVLID
	 * 0x30; Length Override 8 SQLDCPMOD; DRDA TYPE FCS; ENVLID 0x30; Length
	 * Override 5 SQLDCRDB; DRDA TYPE VCS; ENVLID 0x32; Length Override 255
	 * SQLDCTOKS; DRDA TYPE N-RLO; ENVLID 0xF7; Length Override 0 SQLDCMSG_m;
	 * DRDA TYPE NVMC; ENVLID 0x3F; Length Override 32672 SQLDCMSG_S; DRDA TYPE
	 * NVCS; ENVLID 0x33; Length Override 32672 SQLDCCOLN_m; DRDA TYPE NVCM ;
	 * ENVLID 0x3F; Length Override 255 SQLDCCOLN_s; DRDA TYPE NVCS; ENVLID
	 * 0x33; Length Override 255 SQLDCCURN_m; DRDA TYPE NVCM; ENVLID 0x3F;
	 * Length Override 255 SQLDCCURN_s; DRDA TYPE NVCS; ENVLID 0x33; Length
	 * Override 255 SQLDCPNAM_m; DRDA TYPE NVCM; ENVLID 0x3F; Length Override
	 * 255 SQLDCPNAM_s; DRDA TYPE NVCS; ENVLID 0x33; Length Override 255
	 * SQLDCXGRP; DRDA TYPE N-GDA; ENVLID 0xD3; Length Override 1
	 */
	private void writeSQLDCGRP(long rowNum, int sqlCode, String sqlState,
			String dbname, String sqlerrmc) throws DRDAProtocolException {
		// SQLDCCODE
		writer.writeInt(sqlCode);
	 ...
	 * Write SQLDARD
	 *
	 * SQLDARD : FDOCA EARLY ARRAY SQL Descriptor Area Row Description with SQL
	 * Communications Area
	 *
	 * FORMAT FOR SQLAM <= 6 SQLCARD; ROW LID 0x64; ELEMENT TAKEN 0(all); REP
	 * FACTOR 1 SQLNUMROW; ROW LID 0x68; ELEMENT TAKEN 0(all); REP FACTOR 1
	 * SQLDAROW; ROW LID 0x60; ELEMENT TAKEN 0(all); REP FACTOR 0(all)
	 *
	 * FORMAT FOR SQLAM >= 7 SQLCARD; ROW LID 0x64; ELEMENT TAKEN 0(all); REP
	 * FACTOR 1 SQLDHROW; ROW LID 0xE0; ELEMENT TAKEN 0(all); REP FACTOR 1
	 ...
	 * @throws SQLException
	 */
	private void writeSQLDARD(DRDAStatement stmt, boolean rtnOutput,
			SQLException e) throws DRDAProtocolException, SQLException {
		PreparedStatement ps = stmt.getPreparedStatement();
	 ...
	 *
	 * @param stmt
	 *            DRDAStatement we are working on
	 * @param FDODSConly
	 *            simply the FDODSC, without the wrap
	 *
	 *            Instance Variables SQLDTAGRP - required
	 * 
	 *            Only 84 columns can be sent in a single QRYDSC. If there are
	 *            more columns they must be sent in subsequent QRYDSC. If the
	 *            QRYDSC will not fit into the current block, as many columns as
	 *            can fit are sent and then the remaining are sent in the
	 *            following blocks.
	 * 
	 * @throws DRDAProtocolException
	 * @throws SQLException
	...
	private void writeQRYDSC(DRDAStatement stmt, boolean FDODSConly)
		...
		int numCols = (rsmeta != null ? rsmeta.getColumnCount() : pmeta
				.getParameterCount());
		int numGroups = 1;
		int colStart = 1;
		int colEnd = numCols;
		...
		int remaining = blksize - (writer.getDSSLength() % blksize)
				- (3 + FdocaConstants.SQLCADTA_SQLDTARD_RLO_SIZE);
			...
			if (firstcols > FdocaConstants.MAX_VARS_IN_NGDA) {
				if (SanityManager.DEBUG) {
					SanityManager.ASSERT(
							numCols > FdocaConstants.MAX_VARS_IN_NGDA,
				...
				colEnd = firstcols;
				numGroups += (numCols - firstcols)
						/ FdocaConstants.MAX_VARS_IN_NGDA;
				if (FdocaConstants.MAX_VARS_IN_NGDA * numGroups < numCols) {
					numGroups++;
				}
			}
		...
		for (int i = 0; i < numGroups; i++) {
			...
			colEnd = Math
					.min(colEnd + FdocaConstants.MAX_VARS_IN_NGDA, numCols);
		}
	...
	/**
	 * Write SQLDTAGRP SQLDAGRP : Late FDOCA GROUP SQL Data Value Group
	 * Descriptor LENGTH - length of the SQLDTAGRP TRIPLET_TYPE - NGDA for
	 * first, CPT for following ID - SQLDTAGRP_LID for first, NULL_LID for
	 * following For each column DRDA TYPE LENGTH OVERRIDE For numeric/decimal
	 * types PRECISON SCALE otherwise LENGTH or DISPLAY_WIDTH
	 *
	 * @param stmt
	 *            drda statement
	 * @param rsmeta
	 *            resultset meta data
	 * @param pmeta
	 *            parameter meta data for CallableStatement
	 * @param colStart
	 *            starting column for group to send
	 * @param colEnd
	 *            end column to send
	 * @param first
	...
	private void writeSQLDTAGRP(DRDAStatement stmt, ResultSetMetaData rsmeta,
			ParameterMetaData pmeta, int colStart, int colEnd, boolean first)
			throws DRDAProtocolException, SQLException {

		int length = (FdocaConstants.SQLDTAGRP_COL_DSC_SIZE * ((colEnd + 1) - colStart)) + 3;
		...
		boolean hasRs = (rsmeta != null); // if don't have result, then we look
											// at parameter meta

		for (int i = colStart; i <= colEnd; i++) {
			boolean nullable = hasRs ? (rsmeta.isNullable(i) == ResultSetMetaData.columnNullable)
					: (pmeta.isNullable(i) == ParameterMetaData.parameterNullable);
			int colType = (hasRs ? rsmeta.getColumnType(i) : pmeta
					.getParameterType(i));
			int[] outlen = { -1 };
			int drdaType = FdocaConstants.mapJdbcTypeToDrdaType(colType,
					nullable, appRequester, outlen);
			...
			if (hasRs) {
				...
				if (stmt.isOutputParam(i)) {
					...
					((CallableStatement) stmt.ps).registerOutParameter(i,
							Types.DECIMAL, scale);
			...
			if (SanityManager.DEBUG) {
				trace("jdbcType=" + colType + "  \tdrdaType="
	...
	/**
	 * Holdability passed in as it can represent the holdability of the
	 * statement or a specific result set.
	 * 
	 * @param holdability
	 ...
	 * @throws SQLException
	 */
	private void writeSQLDHROW(int holdability) throws DRDAProtocolException,
			SQLException {
		if (JVMInfo.JDK_ID < 2) // write null indicator for SQLDHROW because
								// there is no holdability support prior to
								// jdk1.3
		{
			writer.writeByte(CodePoint.NULLDATA);
			return;
	...
	/**
	 * Write QRYDTA - Query Answer Set Data Contains some or all of the answer
	 * set data resulting from a query If the client is not using rowset
	 * processing, this routine attempts to pack as much data into the QRYDTA as
	 * it can. This may result in splitting the last row across the block, in
	 * which case when the client calls CNTQRY we will return the remainder of
	 * the row.
	 *
	 * Splitting a QRYDTA block is expensive, for several reasons: - extra logic
	 * must be run, on both client and server side - more network round-trips
	 * are involved - the QRYDTA block which contains the continuation of the
	 * split row is generally wasteful, since it contains the remainder of the
	 * split row but no additional rows. Since splitting is expensive, the
	 * server makes some attempt to avoid it. Currently, the server's algorithm
	 * for this is to compute the length of the current row, and to stop trying
	 * to pack more rows into this buffer if another row of that length would
	 * not fit. However, since rows can vary substantially in length, this
	 * algorithm is often ineffective at preventing splits. For example, if a
	 * short row near the end of the buffer is then followed by a long row, that
	 * long row will be split. It is possible to improve this algorithm
	 * substantially: - instead of just using the length of the previous row as
	 * a guide for whether to attempt packing another row in, use some sort of
	 * overall average row size computed over multiple rows (e.g., all the rows
	 * we've placed into this QRYDTA block, or all the rows we've process for
	 * this result set) - when we discover that the next row will not fit,
	 * rather than splitting the row across QRYDTA blocks, if it is relatively
	 * small, we could just hold the entire row in a buffer to place it entirely
	 * into the next QRYDTA block, or reset the result set cursor back one row
	 * to "unread" this row. - when splitting a row across QRYDTA blocks, we
	 * tend to copy data around multiple times. Careful coding could remove some
	 * of these copies. However, it is important not to over-complicate this
	 * code: it is better to be correct than to be efficient, and there have
	 * been several bugs in the split logic already.
	 *
	 * Instance Variables Byte string
	 *
	 * @param stmt
	 *            DRDA statement we are processing
	 * @throws DRDAProtocolException
	 * @throws SQLException
	 */
	private void writeQRYDTA(DRDAStatement stmt) throws DRDAProtocolException,
			SQLException {
		...
		if (!stmt.hasdata()) {
			final boolean qryclsOnLmtblkprc = appRequester
	...
	/**
	 * This routine places some data into the current QRYDTA block using FDODTA
	 * (Formatted Data Object DaTA rules).
	 *
	 * There are 3 basic types of processing flow for this routine: - In normal
	 * non-rowset, non-scrollable cursor flow, this routine places a single row
	 * into the QRYDTA block and returns TRUE, indicating that the caller can
	 * call us back to place another row into the result set if he wishes. (The
	 * caller may need to send Externalized Data, which would be a reason for
	 * him NOT to place any more rows into the QRYDTA). - In ROWSET processing,
	 * this routine places an entire ROWSET of rows into the QRYDTA block and
	 * returns FALSE, indicating that the QRYDTA block is full and should now be
	 * sent. - In callable statement processing, this routine places the results
	 * from the output parameters of the called procedure into the QRYDTA block.
	 * This code path is really dramatically different from the other two paths
	 * and shares only a very small amount of common code in this routine.
	 *
	 * In all cases, it is possible that the data we wish to return may not fit
	 * into the QRYDTA block, in which case we call splitQRYDTA to split the
	 * data and remember the remainder data in the result set. Splitting the
	 * data is relatively rare in the normal cursor case, because our caller
	 * (writeQRYDTA) uses a coarse estimation technique to avoid calling us if
	 * he thinks a split is likely.
	 *
	 * The overall structure of this routine is implemented as two loops: - the
	 * outer "do ... while ... " loop processes a ROWSET, one row at a time. For
	 * non-ROWSET cursors, and for callable statements, this loop executes only
	 * once. - the inner "for ... i < numCols ..." loop processes each column in
	 * the current row, or each output parmeter in the procedure.
	 *
	 * Most column data is written directly inline in the QRYDTA block. Some
	 * data, however, is written as Externalized Data. This is commonly used for
	 * Large Objects. In that case, an Externalized Data Pointer is written into
	 * the QRYDTA block, and the actual data flows in separate EXTDTA blocks
	 * which are returned after this QRYDTA block.
	 */
	private boolean writeFDODTA(DRDAStatement stmt)
		...
		do {
			...
			boolean noRetrieveRS = (rs != null && (!stmt.getQryrtndta() || rs
					.rowDeleted()));
			if (noRetrieveRS) {
			...
			for (int i = 1; i <= numCols; i++) {
				...
				if (rs != null) {
					...
					if (SanityManager.DEBUG) {
						trace("!!drdaType = "
								+ java.lang.Integer.toHexString(drdaType)
					...
					case DRDAConstants.DRDA_TYPE_NINTEGER:
						...
						valNull = rs.wasNull();
						if (SanityManager.DEBUG) {
							trace("====== writing int: " + ival + " is null: "
									+ valNull);
						}
					...
					case DRDAConstants.DRDA_TYPE_NINTEGER8:
						...
						valNull = rs.wasNull();
						if (SanityManager.DEBUG) {
							trace("====== writing long: " + lval + " is null: "
									+ valNull);
						}
						writeNullability(drdaType, valNull);
						if (!valNull) {
					...
					case DRDAConstants.DRDA_TYPE_NLONGMIX:
						...
						writeFdocaVal(i, valStr, drdaType, precision, scale,
								rs.wasNull(), stmt, false);
						break;
					default:
					...
					if (stmt.isOutputParam(i)) {
						...
						drdaType = FdocaConstants.mapJdbcTypeToDrdaType(
								stmt.getOutputParamType(i), true, appRequester,
								outlen);
			...
			/*
			 * (1) scrollable we return at most a row set; OR (2) no retrieve
			 * data
			 */
	...
	/**
	 * <p>
	 * Get a column value of the specified type from a {@code ResultSet}, in a
	 * form suitable for being writted by {@link #writeFdocaVal}. For most
	 ...
	 *
	 * @param rs
	 *            the result set to fetch the object from
	 * @param index
	 *            the column index
	 * @param drdaType
	 ...
	 * {@link #getObjectForWriteFdoca(java.sql.ResultSet, int, int)} for
	 * details.
	 * </p>
	 *
	 * @param cs
	 *            the callable statement to fetch the object from
	 * @param index
	 *            the parameter index
	 * @param drdaType
	 *            the DRDA type of the object to fetch
	 ...
	 */
	private Object getObjectForWriteFdoca(CallableStatement cs, int index,
			int drdaType) throws SQLException {
		// convert to corresponding nullable type to reduce number of cases
		int ndrdaType = drdaType | 1;
		switch (ndrdaType) {
		case DRDAConstants.DRDA_TYPE_NDATE:
	 ...
	 *
	 * This routine is called if the QRYDTA data will not fit. It writes as much
	 * data as it can, then stores the remainder in the result set. At some
	 * later point, when the client returns with a CNTQRY, we will call
	 * processLeftoverQRYDTA to handle that data.
	 *
	 ...
	 * realizes that it has constructed a QRYDTA message which is too large. At
	 * that point, we need to reclaim the "extra" data and hold on to it. To aid
	 * us in that processing, DDMWriter provides the routines getDSSLength,
	 * copyDSSDataToEnd, and truncateDSS. For some additional detail on this
	 * complex sub-protocol, the interested reader should study bug DERBY-491
	 * and 492 at: http://issues.apache.org/jira/browse/DERBY-491 and
	 ...
	 * Normally, it observes that there is no remainder data from the previous
	 * QRYDTA block, and returns FALSE, indicating that there was nothing to do.
	 *
	 * However, if it discovers that the previous QRYDTA block was split, then
	 * it retrieves the remainder data from the result set, writes as much of it
	 * as will fit into the QRYDTA block (hopefully all of it will fit, but the
	 * row may be very long), and returns TRUE, indicating that this QRYDTA
	 * block has been filled with remainder data and should now be sent
	 * immediately.
	 */
	private boolean processLeftoverQRYDTA(DRDAStatement stmt)
		...
		if (leftovers.length < blksize) {
			...
			System.arraycopy(leftovers, blksize, newLeftovers, 0,
					newLeftovers.length);
	...
	private void doneData(DRDAStatement stmt, ResultSet rs)
		...
		// For scrollable cursor's QRYSCRAFT, when we reach here, DRDA spec says
		// sqlstate
		// is 00000, sqlcode is not mentioned. But DB2 CLI code expects sqlcode
		// to be 0.
		...
		writeSQLCAGRP((isQRYSCRAFT ? eod00000 : eod02000), (isQRYSCRAFT ? 0
				: 100), 0, stmt.rowCount);
	...
	private boolean positionCursor(DRDAStatement stmt, ResultSet rs)
		...
		case CodePoint.QRYSCRREL:
			...
			if ((rs.isAfterLast() && rows > 0)
					|| (rs.isBeforeFirst() && rows < 0)) {
				retval = false;
			} else {
		...
		case CodePoint.QRYSCRBEF:
			...
			break;
		default:
			agentError("Invalid value for cursor orientation "
					+ stmt.getQryscrorn());
		}
		return retval;
	}
	...
	/**
	 * Write SQLDAGRP SQLDAGRP : EARLY FDOCA GROUP SQL Data Area Group
	 * Description
	 *
	 * FORMAT FOR SQLAM <= 6 SQLPRECISION; DRDA TYPE I2; ENVLID 0x04; Length
	 * Override 2 SQLSCALE; DRDA TYPE I2; ENVLID 0x04; Length Override 2
	 * SQLLENGTH; DRDA TYPE I4; ENVLID 0x02; Length Override 4 SQLTYPE; DRDA
	 * TYPE I2; ENVLID 0x04; Length Override 2 SQLCCSID; DRDA TYPE FB; ENVLID
	 * 0x26; Length Override 2 SQLNAME_m; DRDA TYPE VCM; ENVLID 0x3E; Length
	 * Override 30 SQLNAME_s; DRDA TYPE VCS; ENVLID 0x32; Length Override 30
	 * SQLLABEL_m; DRDA TYPE VCM; ENVLID 0x3E; Length Override 30 SQLLABEL_s;
	 * DRDA TYPE VCS; ENVLID 0x32; Length Override 30 SQLCOMMENTS_m; DRDA TYPE
	 * VCM; ENVLID 0x3E; Length Override 254 SQLCOMMENTS_m; DRDA TYPE VCS;
	 * ENVLID 0x32; Length Override 254
	 *
	 * FORMAT FOR SQLAM == 6 SQLPRECISION; DRDA TYPE I2; ENVLID 0x04; Length
	 * Override 2 SQLSCALE; DRDA TYPE I2; ENVLID 0x04; Length Override 2
	 * SQLLENGTH; DRDA TYPE I8; ENVLID 0x16; Length Override 8 SQLTYPE; DRDA
	 * TYPE I2; ENVLID 0x04; Length Override 2 SQLCCSID; DRDA TYPE FB; ENVLID
	 * 0x26; Length Override 2 SQLNAME_m; DRDA TYPE VCM; ENVLID 0x3E; Length
	 * Override 30 SQLNAME_s; DRDA TYPE VCS; ENVLID 0x32; Length Override 30
	 * SQLLABEL_m; DRDA TYPE VCM; ENVLID 0x3E; Length Override 30 SQLLABEL_s;
	 * DRDA TYPE VCS; ENVLID 0x32; Length Override 30 SQLCOMMENTS_m; DRDA TYPE
	 * VCM; ENVLID 0x3E; Length Override 254 SQLCOMMENTS_m; DRDA TYPE VCS;
	 * ENVLID 0x32; Length Override 254 SQLUDTGRP; DRDA TYPE N-GDA; ENVLID 0x51;
	 * Length Override 0
	 *
	 * FORMAT FOR SQLAM >= 7 SQLPRECISION; DRDA TYPE I2; ENVLID 0x04; Length
	 * Override 2 SQLSCALE; DRDA TYPE I2; ENVLID 0x04; Length Override 2
	 * SQLLENGTH; DRDA TYPE I8; ENVLID 0x16; Length Override 8 SQLTYPE; DRDA
	 * TYPE I2; ENVLID 0x04; Length Override 2 SQLCCSID; DRDA TYPE FB; ENVLID
	 * 0x26; Length Override 2 SQLDOPTGRP; DRDA TYPE N-GDA; ENVLID 0xD2; Length
	 * Override 0
	 *
	 * @param rsmeta
	 *            resultset meta data
	...
	private void writeSQLDAGRP(ResultSetMetaData rsmeta,
			ParameterMetaData pmeta, int elemNum, boolean rtnOutput)
			throws DRDAProtocolException, SQLException {
		...
		int elemType = rtnOutput ? rsmeta.getColumnType(jdbcElemNum) : pmeta
				.getParameterType(jdbcElemNum);

		int precision = Math.min(
				FdocaConstants.NUMERIC_MAX_PRECISION,
				rtnOutput ? rsmeta.getPrecision(jdbcElemNum) : pmeta
						.getPrecision(jdbcElemNum));

		// 2-byte precision
		writer.writeShort(precision);
		// 2-byte scale
		int scale = (rtnOutput ? rsmeta.getScale(jdbcElemNum) : pmeta
				.getScale(jdbcElemNum));
		writer.writeShort(scale);

		boolean nullable = rtnOutput ? (rsmeta.isNullable(jdbcElemNum) == ResultSetMetaData.columnNullable)
				: (pmeta.isNullable(jdbcElemNum) == ParameterMetaData.parameterNullable);

		int sqlType = SQLTypes.mapJdbcTypeToDB2SqlType(elemType, nullable,
				appRequester, outlen);
		...
		if (outlen[0] == -1) // some types not set
		{
			...
			case Types.DECIMAL:
			case Types.NUMERIC:
				scale = rtnOutput ? rsmeta.getScale(jdbcElemNum) : pmeta
						.getScale(jdbcElemNum);
		...
		case Types.CLOB:
			outlen[0] = (rtnOutput ? rsmeta.getPrecision(jdbcElemNum) : pmeta
					.getPrecision(jdbcElemNum));
		}

		if (SanityManager.DEBUG) {
			trace("SQLDAGRP len =" + java.lang.Integer.toHexString(outlen[0])
					+ "for type:" + elemType);
		}
		...
		if (SanityManager.DEBUG) {
			trace("jdbcType =" + typeName + "  sqlType =" + sqlType + "len ="
					+ outlen[0]);
		}
	...
	/**
	 * Write variable character mixed byte or single byte The preference is to
	 * write mixed byte if it is defined for the server, since that is our
	 * default and we don't allow it to be changed, we always write mixed byte.
	 * 
	 * @param s
	 *            string to write
	 * @exception DRDAProtocolException
	 ...
	 * this format does not convey the Java class name needed by
	 * ResultSetMetaData.getColumnClassName().
	 *
	 * SQLUDXTYPE; DRDA TYPE I4; ENVLID 0x02; Length Override 4 Constants which
	 * map to java.sql.Types constants DISTINCT, STRUCT, and REF. But DRDA does
	 * not define a constant which maps to java.sql.Types.JAVA_OBJECT.
	 * SQLUDTRDB; DRDA TYPE VCS; ENVLID 0x32; Length Override 255 Database name.
	 * SQLUDTSCHEMA_m; DRDA TYPE VCM; ENVLID 0x3E; Length Override 255
	 * SQLUDTSCHEMA_s; DRDA TYPE VCS; ENVLID 0x32; Length Override 255 Schema
	 * name. One of the above. SQLUDTNAME_m; DRDA TYPE VCM; ENVLID 0x3E; Length
	 * Override 255 SQLUDTNAME_s; DRDA TYPE VCS; ENVLID 0x32; Length Override
	 * 255 Unqualified UDT name. One of the above.
	 *
	 ...
	 * SQLUDTNAME_s; DRDA TYPE VCS; ENVLID 0x32; Length Override 255 Fully
	 * qualified UDT name. One of the above. SQLUDTCLASSNAME_m; DRDA TYPE VCM;
	 * ENVLID 0x3E; Length Override FdocaConstants.LONGVARCHAR_MAX_LEN
	 * SQLUDTCLASSNAME_s; DRDA TYPE VCS; ENVLID 0x32; Length Override
	 * FdocaConstants.LONGVARCHAR_MAX_LEN Name of the Java class bound to the
	 * UDT. One of the above.
	 *
	 * @param rsmeta
	 *            resultset meta data
	 * @param pmeta
	...
	private void writeSQLUDTGRP(ResultSetMetaData rsmeta,
			ParameterMetaData pmeta, int jdbcElemNum, boolean rtnOutput)
			throws DRDAProtocolException, SQLException {
		int jdbcType = rtnOutput ? rsmeta.getColumnType(jdbcElemNum) : pmeta
				.getParameterType(jdbcElemNum);
		...
		String typeName = rtnOutput ? rsmeta.getColumnTypeName(jdbcElemNum)
				: pmeta.getParameterTypeName(jdbcElemNum);
		String className = rtnOutput ? rsmeta.getColumnClassName(jdbcElemNum)
	...
	private void writeSQLDXGRP(ResultSetMetaData rsmeta,
		...
		if (pmeta != null && !rtnOutput) {
			...
			if (mode == ParameterMetaData.parameterModeUnknown) {
				...
				int type = DRDAStatement
						.getOutputParameterTypeFromClassName(pmeta
								.getParameterClassName(jdbcElemNum));
	 ...
	 * @param val
	 *            Value to write to client
	 * @param drdaType
	 *            FD:OCA DRDA Type from FdocaConstants
	 * @param precision
	 *            Precision
	 * @param stmt
	 *            Statement being processed
	 * @param isParam
	 *            True when writing a value for a procedure parameter
	 *
	 * @exception DRDAProtocolException
	 * 
	...
	protected void writeFdocaVal(int index, Object val, int drdaType,
			int precision, int scale, boolean valNull, DRDAStatement stmt,
			boolean isParam) throws DRDAProtocolException, SQLException {
		writeNullability(drdaType, valNull);
	...
	/**
	 * write nullability if this is a nullable drdatype and FDOCA null value if
	 * appropriate
	 * 
	 * @param drdaType
	 *            FDOCA type
	...
	/**
	 * Convert a {@code java.sql.Date} to a string with the format expected by
	 * the client.
	 *
	 * @param date
	 *            the date to format
	...
	/**
	 * Convert a {@code java.sql.Time} to a string with the format expected by
	 * the client.
	 *
	 * @param time
	 *            the time to format
	...
	/**
	 * Convert a {@code java.sql.Timestamp} to a string with the format expected
	 * by the client.
	 *
	 * @param ts
	...
	/**
	 * Insert an integer into a char array and pad it with leading zeros if its
	 * string representation is shorter than {@code length} characters.
	 *
	 * @param buf
	 *            the char array
	 * @param offset
	 *            where in the array to start inserting the value
	 * @param length
	 *            the desired length of the inserted string
	 * @param value
	...
	/**
	 * Invalid non-derby client tried to connect. thrown a required Value not
	 * found error and log a message to derby.log
	 * 
	 * @param prdid
	 *            product id that does not match DNC
	 * @throws DRDAProtocolException
	 */
	...
	private void rdbNotFound(String rdbnam) throws DRDAProtocolException {
		Object[] oa = { rdbnam };
		throw new DRDAProtocolException(
				DRDAProtocolException.DRDA_Proto_RDBNFNRM, this, 0,
	...
	protected void codePointNotSupported(int codePoint)
			throws DRDAProtocolException {
		throw new DRDAProtocolException(
				DRDAProtocolException.DRDA_Proto_CMDNSPRM, this, codePoint,
	...
	private void valueNotSupported(int codePoint) throws DRDAProtocolException {
		throw new DRDAProtocolException(
				DRDAProtocolException.DRDA_Proto_VALNSPRM, this, codePoint,
				DRDAProtocolException.NO_ASSOC_ERRCD);
	}
	...
	/**
	 * Database name given under code point doesn't match previous database
	 * names
	 *
	 * @param codePoint
	...
	private void closeSession() {
		...
		/*
		 * DERBY-2220: Rollback the current XA transaction if it is still
	 ...
	 *
	 * @param de
	 *            <code>DRDAProtocolException</code> to be sent
	 */
	...
	private void sendUnexpectedException(Exception e) {
		...
		try {
			...
			unExpDe = DRDAProtocolException.newAgentError(this,
					CodePoint.SVRCOD_PRMDMG, dbname, e.getMessage());
	 ...
	 *
	 * @param drdaType
	 *            the DRDA type of the EXTDTA value
	 * @param index
	...
	private void traceEXTDTARead(int drdaType, int index,
			EXTDTAReaderInputStream stream, boolean streamLOB, String encoding) {
		if (SanityManager.DEBUG && server.debugOutput() == true) {
			...
			sb.append("t").append(drdaType).append("/i").append(index)
					.append("/").append(streamLOB).append("/").append(encoding)
					.append("/").append(stream.readStatusByte).append("/b");
			if (stream == null) {
				sb.append("NULL");
			} else if (stream.isLayerBStream()) {
				sb.append("UNKNOWN_LENGTH");
			} else {
				sb.append(((StandardEXTDTAReaderInputStream) stream)
						.getLength());
			}
			trace(sb.toString());
		}
	...
	private void checkValidTypDefNam(String typdefnam)
			throws DRDAProtocolException {
		if (!(typdefnam.equals("QTDSQL370") || typdefnam.equals("QTDSQL400")
				|| typdefnam.equals("QTDSQLX86")
				|| typdefnam.equals("QTDSQLASC")
				|| typdefnam.equals("QTDSQLVAX") || typdefnam
					.equals("QTDSQLJVM"))) {
	 ...
	 * 
	 * @param dbname
	 *            database name to initialize. If dbnam is non null, add
	...
	/**
	 * Write ENDUOWRM Instance Variables SVCOD - severity code - WARNING -
	 * required UOWDSP - Unit of Work Disposition - required RDBNAM - Relational
	 * Database name - optional SRVDGN - Server Diagnostics information -
	 * optional
	 *
	 * @param opType
	...
	void writeEXTDTA(DRDAStatement stmt) throws SQLException,
			DRDAProtocolException {
		...
		for (int i = 0; i < extdtaValues.size(); i++) {
			...
			if (i != extdtaValues.size() - 1) { // no
				...
				chainFlag = false; // last blob DSS stream itself is NOT chained
									// with the NEXT DSS
			...
			if (o instanceof EXTDTAInputStream) {
				...
				try {
					...
					writer.writeScalarStream(chainedWithSameCorrelator,
							CodePoint.EXTDTA, stream, writeNullByte);
	 ...
	 *            result set to check
	 * @param updateCount
	 *            update count to include in SQLCARD
	 * @param alwaysSend
	 *            whether always send SQLCARD regardless of the existance of
	 *            warnings
	 * @param sendWarn
	...
	private void checkWarning(Connection conn, Statement stmt, ResultSet rs,
			...
			throws DRDAProtocolException, SQLException {
		// instead of writing a chain of sql warning, we send the first one,
		// this is
		// jcc/db2 limitation, see beetle 4629
	...
	protected String buildRuntimeInfo(String indent,
	 ...
	 * security mechanism based on the environment, application requester's
	 * identity (PRDID) and connection URL.
	 *
	 * IMPORTANT NOTE: -------------- SECMEC_USRSSBPWD is ONLY supported by the
	 * target server if: - current authentication provider is Derby BUILTIN or
	 * NONE. (database / system level) (Phase I) - database-level password must
	 * have been encrypted with the SHA-1 based authentication scheme -
	 * Application requester is 'DNC' (Derby Network Client) (Phase I)
	 *
	 * @return security check code - 0 if everything O.K.
	 */
	private int validateSecMecUSRSSBPWD() throws DRDAProtocolException {
		...
		appRequester.setClientVersion(srvrlslv.substring(0,
				(int) CodePoint.PRDID_MAX));
		...
		if ((dbName == null) || (dbName.length() == 0)) {
			...
			// the engine.
			authenticationService = ((InternalDriver) NetworkServerControlImpl
					.getDriver()).getAuthenticationService();
		} else {
			...
			if (Monitor.getMonitor() != null) {
				databaseObj = (org.apache.derby.iapi.db.Database) Monitor
			...
			try {
				if (databaseObj != null) {
					authenticationService = databaseObj
		...
		if (authenticationService != null) {
			...
			if (!authClassName.equals(AUTHENTICATION_PROVIDER_BUILTIN_CLASS)
					&& !authClassName
							.equals(AUTHENTICATION_PROVIDER_NONE_CLASS)) {
				return CodePoint.SECCHKCD_NOTSUPPORTED; // Not Supported
			}
		}
	 ...
	 *
	 * @param stream
	 *            the stream to close (possibly {@code null})
	 * @throws SQLException
	 *             wrapped around an {@code IOException} if closing the stream
	 *             failed
	 */
	...
	private static InputStream convertAsByteArrayInputStream(
			EXTDTAReaderInputStream stream) throws IOException {
		...
		final int byteArrayLength = stream instanceof StandardEXTDTAReaderInputStream ? (int) ((StandardEXTDTAReaderInputStream) stream)
				.getLength() : 1 + stream.available(); // +1 to avoid infinite
														// loop
		...
		if (stream.isStatusSet()
			...
			return new ByteArrayInputStream(pbos.getBuffer(), 0,
					pbos.getCount());
		}
	 ...
	 *
	 * @param stmt
	 *            the DRDA statement to use
	 * @param i
	 *            the one-based index of the parameter
	 * @param extdtaStream
	 *            the EXTDTA stream to read data from
	 * @param streamLOB
	 *            whether or not the stream content is streamed as the last
	 *            value in the DRDA protocol flow
	 * @param encoding
	 ...
	 */
	private static void setAsCharacterStream(DRDAStatement stmt, int i,
			EXTDTAReaderInputStream extdtaStream, boolean streamLOB,
			String encoding) throws IOException, SQLException {
		PreparedStatement ps = stmt.getPreparedStatement();
		...
		final InputStream is = streamLOB ? (InputStream) extdtaStream
	 ...
	 *             if setting the stream fails
	 */
	private static void setAsBinaryStream(DRDAStatement stmt, int index,
			EXTDTAReaderInputStream stream, boolean streamLOB)
		...
		if (streamLOB && useSetBinaryStream) {
			...
			if (stream == null) {
				...
				int length = (int) ((StandardEXTDTAReaderInputStream) stream)
						.getLength();
