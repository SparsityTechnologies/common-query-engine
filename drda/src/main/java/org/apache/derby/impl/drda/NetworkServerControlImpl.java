...
public class NetworkServerControlImpl {
        ...
        synchronized (serverStartSync) {
            ...
            // should still work correctly, just not as efficiently...))
                ...
                try {
                    ...
                    // start the server.
                    cloudscapeDriver = (Driver) Class.forName(getCloudScapeDriver()).newInstance();
    ...
    */
        ...
        String password) throws Exception
    {
        ...
        try {
            Class.forName(getCloudScapeDriver());
        }
        catch (Exception e) {
            sendMessage(writer, ERROR, e.getMessage());
            return;
    ...
    public DRDAConnThread createConnectionThread(Session session){
    	return  new DRDAConnThread(session, this, getTimeSlice(),
                getLogConnections());
    }
        ...
        if (!enoughThreads) {
            ...
            synchronized (threadsSync) {
                ...
                if ((maxThreads == 0) || (threadList.size() < maxThreads)) {
                    thread = createConnectionThread(session);
