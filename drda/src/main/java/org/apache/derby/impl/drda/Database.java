...
/**
 * Database stores information about the current database It is used so that a
 * session may have more than one database
 */
public class Database {
	/*
	 * Considering that we are now doing some fiddling with the db name it is
	 * probably wise to keep dbName and shortDbName private and have accessors
	 * for them.
	 */
	...
	protected byte[] passwordSubstitute;// password substitute -
										// SECMEC_USRSSBPWD
	protected boolean rdbAllowUpdates = true; // Database allows updates
												// -default is true
	...
	protected DRDAStatement defaultStatement; // default statement used
												// for execute imm
	private DRDAStatement currentStatement; // current statement we are working
											// on
	/** Hash table for storing statements. */
	private Hashtable<Object, DRDAStatement> stmtTable;

	private String queryLanguage = "cloudmdsql";
	...
	/**
	 * Take database name including attributes and set attrString and
	 * shortDbName accordingly.
	 * 
	 * @param dbName
	...
	public void setDatabaseName(String dbName) {
		if (dbName != null) {
			// dbName = seconddb;create=true;queryLang=cloudMdSQL
			...
			if (attrOffset != -1) {
				...
				String[] prop = attrString.split(";");
				for (int i = 0; i < prop.length; i++) {
					if (prop[i].startsWith("queryLang")) {
						String[] values = prop[i].split("=");
						if (values.length == 2) {
							String lang = values[1].trim().toLowerCase();
	 ...
	 */
	 public void setConnection(EngineConnection conn) throws SQLException {
		if (this.conn != conn) {
			// Need to drop the pb session data when switching connections
			pbsd_ = null;
		}
		this.conn = conn;
		this.conn.setQueryLanguage(queryLanguage);
		if (conn != null)
	...
	/**
	 * Get default statement for use in EXCIMM with specified pkgnamcsn The
	 * pkgnamcsn has the encoded isolation level
	 *
	 * @param pkgnamcsn
	 ...
	 * new statement with a previously used pkgnamcsn, it means that client-side
	 * statement associated with this pkgnamcsn has been closed. In this case,
	 * server can re-use the DRDAStatement by doing the following: 1) Retrieve
	 * the old DRDAStatement associated with this pkgnamcsn and close it. 2)
	 * Reset the DRDAStatement state for re-use.
	 * 
	 * @param pkgnamcsn
	 *            Package name and section
	 * @return DRDAStatement
	 */
	public DRDAStatement newDRDAStatement(Pkgnamcsn pkgnamcsn)
			throws SQLException {
	...
	protected DRDAStatement getDRDAStatement(Pkgnamcsn pkgnamcsn) {
		DRDAStatement newStmt = (DRDAStatement) stmtTable.get(pkgnamcsn
				.getStatementKey());
		if (newStmt != null) {
	...
	/**
	 * Make a new connection using the database name and set the connection in
	 * the database
	 * 
	 * @param p
	 *            Properties for connection attributes to pass to connect
	 */
	public void makeConnection(Properties p) throws SQLException {
		...
		EngineConnection conn = (EngineConnection) NetworkServerControlImpl
				.getDriver().connect(
						Attribute.PROTOCOL + shortDbName + attrString, p);
		if (conn != null) {
			conn.setAutoCommit(false);
			conn.setQueryLanguage(queryLanguage);
		}
		setConnection(conn);
	}

	/**
	 * This makes a dummy connection to the database in order to boot and/or
	 * create this last one. If database cannot be found or authentication does
	 * not succeed, this will throw a SQLException which we catch and do
	 * nothing. We don't pass a userid and password here as we don't need to for
	 * the purpose of this method - main goal is to cause the database to be
	...
	void makeDummyConnection() {
		try {
			...
			EngineConnection conn = (EngineConnection) NetworkServerControlImpl
					.getDriver().connect(
							Attribute.PROTOCOL + shortDbName + attrString,
							new Properties());
	...
	/**
	 * Database close does following cleanup tasks 1)Rollback any pending
	 * transaction on the Connection object (except for a global-XA Connection
	 * obejct) before closing the Connection. Without the rollback, the
	 * Connection close will result into an exception if there is a pending
	 * transaction on that Connection. 2)Clean up the statement table
	 * 
	 * @throws SQLException
	...
	protected String buildRuntimeInfo(String indent,
		...
		String s = indent
				...
				+ "\n"
				+ localLangUtil
						.getTextMessage("DRDA_RuntimeInfoNumStatements.I")
				+ stmtTable.size() + "\n";
		s += localLangUtil
				.getTextMessage("DRDA_RuntimeInfoPreparedStatementHeader.I");
		for (Enumeration e = stmtTable.elements(); e.hasMoreElements();) {
	 ...
	 * whether one of the stored procedures needed for locators exists. (If the
	 * database has been soft-upgraded from an earlier version, the procedures
	 * will not exist).
	 *
	 * @throws SQLException
	 *             if metadata call fails
	 * @return <code>true</code> if locators are supported, <code>false</code
	 *         otherwise
	 */
	boolean supportsLocator() throws SQLException {
		if (!locatorSupportChecked) {
			// Check if locator procedures exist
			ResultSet rs = getConnection().getMetaData().getProcedures(null,
					"SYSIBM", "BLOBTRUNCATE");
	...
	/**
	 * This method resets the state of this Database object so that it can be
	 * re-used. Note: currently this method resets the variables related to
	 * security mechanisms that have been investigated as needing a reset. TODO:
	 * Investigate what all variables in this class need to be reset when this
	 * database object is re-used on a connection pooling or transaction
	 * pooling. see DRDAConnThread.parseACCSEC (CodePoint.RDBNAM) where database
	 * object is re-used on a connection reset.
	 */
	public void reset() {
	...
	/**
	 * Piggy-backed session data. Null if no piggy-backing has happened yet.
	 * Lazy initialization is acceptable since the client's cache initially is
	 * empty so that any request made prior to the first round of piggy-backing
	 * will trigger an actual request to the server.
	 */
