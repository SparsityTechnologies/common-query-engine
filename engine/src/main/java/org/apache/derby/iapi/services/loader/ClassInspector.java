 ...
 * All class names within this interface are treated as java language class
 * names, e.g. int, COM.foo.Myclass, int[], java.lang.Object[]. That is java
 * internal class names as defined in the class file format are not understood.
 */
public class ClassInspector {
	private static final String[] primTypeNames = { "boolean", "byte", "char", "short", "int", "long", "float",
			"double" };

	// collect these as static, instead of each time allocates these new
	// Strings for every method resolution

	private static final String[] nonPrimTypeNames = { "java.lang.Boolean", "java.lang.Byte", "java.lang.Character",
	 ...
	 * @param obj
	 *            The object to test to see if it's an instance of the named
	 *            class
	 *
	 ...
	 *
	 * @param fromClassName
	 *            The name of the class to be assigned
	 * @param toClassName
	 *            The name of the class to be assigned to
	 *
	 * @return true if an object of type fromClass can be assigned to an object
	 *         of type toClass, false if not.
	 */
	...
	/**
	 * Get the Java name of the return type from a Member representing a method
	 * or the type of a Member representing a field.
	 *
	 * @param member
	 *            A Member representing the method for which we want the return
	 *            type.
	 *
	 * @return A Java-language-style string describing the return type of the
	 *         method (for example, it returns "int" instead of "I".
	 */
	...
	/**
	 * Find a public method that implements a given signature. The signature is
	 * given using the full Java class names of the types. <BR>
	 * A untyped null parameter is indicated by passing in an empty string ("")
	 * as its class name. <BR>
	 * If receiverType represents an interface then the methods of
	 * java.lang.Object are included in the candidate list. <BR>
	 ...
	 * the validity of a method alias when creating one.) <BR>
	 * We use a two-pass algorithm to resolve methods. In the first pass, we use
	 * all "object" types to try to match a method. If this fails, in the second
	 * pass, an array of "primitive" types (if the parameter has one, otherwise
	 * the same object type is used) is passed in, as well as the "object" type
	 * array. For each parameter of a method, we try to match it against either
	 * the "object" type, or the "primitive" type. Of all the qualified
	 * candidate methods found, we choose the closest one to the input parameter
	 * types. This involves comparing methods whose parameters are mixed
	 * "object" and "primitive" types in the second pass. This is eventually
	 * handled in classConvertableFromTo.
	 *
	 * @param receiverType
	 ...
	 * @param parmTypes
	 *            An array of class names representing the parameter types. Pass
	 *            a zero-element array if there are no parameters. Pass a null
	 *            if it is okay to match any signature.
	 * @param primParmTypes
	 *            This is used in the second pass of the two-pass method
	 *            resolution algorithm. Use primitive type if it has one,
	 *            otherwise use same object type
	 * @param isParam
	 *            Array of booleans telling whether parameter is a ?.
	 * @param staticMethod
	 *            Find a static method.
	 * @param repeatLastParameter
	 *            If true the last parameter may be repeated any number of times
	 *            (total count must be greater than one). If false the last
	 *            parameter is matched as usual. This also requires an exact
	 *            match on the last parameter type.
	 *
	 * @return A Member representing the matching method. Returns null if no
	 *         such method.
	 *
	 * @exception ClassNotFoundException
	 *                One or more of the classes does not exist.
	 * @exception StandardException
	 *                Thrown on ambiguous method invocation.
	 *
	 * @see Member
	 * @see Modifier
	 */
	public Member findPublicMethod(String receiverType, String methodName, String[] parmTypes, String[] primParmTypes,
			boolean[] isParam, boolean staticMethod, boolean repeatLastParameter, boolean hasVarargs)
		...
		if (parmTypes == null) {
			...
			for (int index = 0; index < methods.length; index++) {
				...
				if (hasVarargs && !isVarArgsMethod(methods[index])) {
					continue;
				}
		...
		if (paramClasses.length == 0) {
			...
			try {
				...
				// if we are an interface then the method could be defined on
				// Object
		...
		// if we have an interface we need to add the methods of Object into the
		// mix
		if (receiverClass.isInterface()) {
		...
		return resolveMethod(receiverClass, methodName, paramClasses, primParamClasses, isParam, staticMethod,
				repeatLastParameter, methodList, hasVarargs);
	}

	/**
	 * Find a public field for a class. This follows the semantics of the java
	 * compiler for locating a field. This means if a field fieldName exists in
	 * the class with package, private or protected then an error is raised.
	 * Even if the field hides a field fieldName in a
	 * super-class/super--interface. See the JVM spec on fields.
	 *
	 * @param receiverType
	 *            The class name of the receiver
	 * @param fieldName
	 ...
	 * @exception StandardException
	 *                Class or field does not exist or is not public or a
	 *                security exception.
	 *
	 * @see Member
	 * @see Modifier
	...
	public Member findPublicField(String receiverType, String fieldName, boolean staticField) throws StandardException {
		...
		try {
			...
			if (receiverClass.isArray() || receiverClass.isPrimitive()) {
				// arrays don't have fields (the fake field 'length' is not
				// returned here)
				return null;
			}
			...
			if ((publicField.getModifiers() & modifier) == modifier) {
				/*
				 * If the class is an interface then we avoid looking for a
				 * declared field that can hide a super-class's public field and
				 * not be accessable. This is because a interface's fields are
				 * always public. This avoids a security check.
				 */
				if (receiverClass.isInterface() || (publicField.getDeclaringClass().equals(receiverClass)))
					return publicField;

				/*
				 * Now check to see if there is a declared field that hides the
				 * public field.
				 */
		...
		throw StandardException.newException(
				staticField ? SQLState.LANG_NO_STATIC_FIELD_FOUND : SQLState.LANG_NO_FIELD_FOUND, e, fieldName,
				receiverType);
	}

	/**
	 * Find a public constructor that implements a given signature. The
	 * signature is given using the full Java class names of the types. <BR>
	 * A untyped null parameter is indicated by passing in an empty string ("")
	 * as its class name.
	 *
	 * @param receiverType
	 ...
	 * @param parmTypes
	 *            An array of class names representing the parameter types. Pass
	 *            a zero-element array if there are no parameters.
	 * @param primParmTypes
	 *            This is used in the second pass of the two-pass method
	 *            resolution algorithm. Use primitive type if it has one,
	 *            otherwise use same object type
	 ...
	 *            Array of booleans telling whether parameter is a ?.
	 *
	 * @return A Member representing the matching constructor. Returns null if
	 *         no such constructor.
	 *
	 * @exception ClassNotFoundException
	 *                One or more of the classes does not exist.
	 * @exception StandardException
	 *                Thrown on ambiguous constructor invocation.
	 *
	 * @see Member
	 * @see Modifier
	 */
	public Member findPublicConstructor(String receiverType, String[] parmTypes, String[] primParmTypes,
			boolean[] isParam) throws ClassNotFoundException, StandardException {
		Class<?> receiverClass = getClass(receiverType);
		if (receiverClass == null)
			return null;
		...
		return resolveMethod(receiverClass, "<init>", paramClasses, primParamClasses, isParam, false, false,
				receiverClass.getConstructors(), true);
	}

	/**
	 * Given an implementation of a parameterized interface, return the bounds
	 * on the type variables. May return null if type resolution fails.
	 */
	...
	/**
	 * Given an implementation of a parameterized interface, return the actual
	 * types of the interface type variables. May return null or an array of
	 * nulls if type resolution fails.
	 */
	...
	/**
	 * Tricky function to resolve a method. If primParamClasses is null we know
	 * it's first pass. First pass try to match as all "object" types, second
	 * pass try to match any combination of "object" and "primitive" types. Find
	 * the closest match among all the qualified candidates. If there's a tie,
	 * it's ambiguous.
	 *
	 ...
	 * DERBY-3652, we now allow primitive and wrapper type matches during the
	 * first pass. The ANSI rules are documented in DERBY-3652.
	 *
	 * @param receiverClass
	 *            the class who holds the methods
	 * @param methodName
	 *            the name of method
	 * @param paramClasses
	 *            object type classes of input parameters
	 * @param primParamClasses
	 *            primitive type classes or null
	 * @param isParam
	 *            isParam (for ?) array
	 * @param staticMethod
	 ...
	 *
	 **/
	private Member resolveMethod(Class receiverClass, String methodName, Class[] paramClasses, Class[] primParamClasses,
			boolean[] isParam, boolean staticMethod, boolean repeatLastParameter, Member[] methods, boolean hasVarargs)
					throws StandardException {

		if (SanityManager.DEBUG) {
			if (SanityManager.DEBUG_ON("MethodResolutionInfo")) {
				SanityManager.DEBUG("MethodResolutionInfo", "MRI - Begin method resolution trace for " + methodName
						+ "() with " + paramClasses.length + (repeatLastParameter ? "+" : "") + " parameters");

				for (int parmCtr = 0; parmCtr < paramClasses.length; parmCtr++) {
					SanityManager.DEBUG("MethodResolutionInfo", "MRI - Parameter #" + parmCtr + " is of type "
							+ (paramClasses[parmCtr] == null ? "null" : paramClasses[parmCtr].getName()));
				}
			}
		}
		...
		do {
			...
			nextMethod: for (int i = 0; i < methods.length; i++) {
				...
				if ((currentMethod == null) || (i == candidateIndex)) {
					continue;
				}
				...
				if (firstTimeAround) {
					...
					if (repeatLastParameter) {
						...
						if (!isVarArgsMethod(methods[i]) && currentMethodParameters.length != paramClasses.length) {
							methods[i] = null; // remove non-applicable methods
							continue;
						}
					}
					...
					/*
					// If the routine was declared to be varargs, then we
					// eliminate
					// all non-varargs methods from consideration
					...
					if (repeatLastParameter) {
						// With N parameters requested check all parameters from
						// N-1 to end are equal
						...
						for (int pr = paramClasses.length - 1; pr < currentMethodParameters.length; pr++) {
							if (!currentMethodParameters[pr].equals(paramClasses[paramClasses.length - 1])) {
								methods[i] = null; // remove non-applicable
													// methods
								continue nextMethod;
							}
						}
				...
				//
				// can the required signature be converted to those of this
				// method?
				if (currentMethod instanceof Method) {
					Method aux = (Method) currentMethod;
					if (aux.isVarArgs()) {
						// we need to compare the non var params and compare the
						// others with the array's container class
						Class<?>[] NonVarmethodParams = new Class[currentMethodParameters.length - 1];
						Class<?>[] NonVarParamClasses = new Class[currentMethodParameters.length - 1];
						for (int j = 0; j < currentMethodParameters.length - 1; j++) {
							NonVarmethodParams[j] = currentMethodParameters[j];
							NonVarParamClasses[j] = paramClasses[j];
						}
						Class<?> arrayType = currentMethodParameters[currentMethodParameters.length - 1]
								.getComponentType();
						boolean paramsAreSubclass = true;
						for (int k = currentMethodParameters.length - 1; k < paramClasses.length
								&& paramsAreSubclass; k++) {
							paramsAreSubclass = Object.class.equals(arrayType)
									|| classConvertableFromTo(arrayType, paramClasses[k], true);
						}
						if (paramsAreSubclass) {
							currentMethodParameters = NonVarmethodParams;
							paramClasses = NonVarParamClasses;
						} else {
							methods[i] = null; // remove non-applicable methods
							continue;
						}
				...
				if (!signatureConvertableFromTo(paramClasses, primParamClasses, currentMethodParameters, isParam,
						true)) {
					...
					if (isVarArgsMethod(currentMethod) && paramClasses.length >= currentMethodParameters.length) {
						boolean valid = true;

						for (int k = 0; k < currentMethodParameters.length - 1 && valid; k++) {
							valid = currentMethodParameters[k].isAssignableFrom(paramClasses[k]);
						}
						if (valid) {
							Class<?> component = currentMethodParameters[currentMethodParameters.length - 1]
									.getComponentType();
							for (int k = currentMethodParameters.length - 1; k < paramClasses.length && valid; k++) {
								valid = component.isAssignableFrom(paramClasses[k]);
							}
						}
						if (!valid) {
							methods[i] = null;
				...
				/*
				 * We have seen an ambiguous situation; one of the cases may tie
				 * on each parameter.
				 */
				ambiguous = true;
		...
		if (ambiguous) {
			...
			throw StandardException.newException(SQLState.LANG_AMBIGUOUS_METHOD_INVOCATION, receiverClass.getName(),
					methodName, parmTypesString.toString());
		}
	...
	/**
	 * Get (load) the class for the given class name. This method converts any
	 * java language class name into a Class object. This includes cases like
	 * String[] and primitive types. This will attempt to load the class from
	 * the application set.
	 * 
	 * @exception ClassNotFoundException
	 *                Class cannot be found, or a SecurityException or
	 ...
	 * possible; this is used for comparing two candidate methods in the second
	 * pass of the two pass method resolution.
	 *
	 * @param fromTypes
	 *            from types' classes
	 * @param primFromTypes
	 *            primitive from types or null
	 * @param toTypes
	 *            to types' classes
	 * @param isParam
	 *            is parameter (?) or not
	 ...
	 **/
	private boolean signatureConvertableFromTo(Class[] fromTypes, Class[] primFromTypes, Class[] toTypes,
			boolean[] isParam, boolean mixTypes) {

		// In the case repeatLastParameter was true, then the two methods may
		// have
		// different numbers of parameters. We need to compare only the
		// non-repeated
		...
		for (int i = 0; i < checkCount; i++) {
			...
			// the caller of findPublicMethod. Any signatures of existing
			// methods
			// are always typed.
			if (fromClass == null) {
	...
	/**
	 * Can we convert a fromClass to toClass. "mixTypes" is a flag to show if
	 * object/primitive type conversion is possible; this is used for comparing
	 * two candidate methods in the second pass of the two pass method
	 * resolution.
	 *
	 * @param fromClass
	 *            from class
	...
	protected boolean classConvertableFromTo(Class fromClass, Class toClass, boolean mixTypes) {
		...
		//
		// OUT and INOUT args are arrays. Compare the cell types rather than the
		// array types.
		//
		...
		if(toClass.getName().equals(Object.class.getName())){
			return true;
		}
		...
		if ((fromClass == Boolean.TYPE) || fromName.equals(nonPrimTypeNames[0])) {
			...
			if ((toClass == Byte.TYPE) || toName.equals(nonPrimTypeNames[1]) ||
					// we never need to see if toClass is of wider "object"
					// type,
					// because a wider "object" type and a narrower "primitive"
					// type can never both be candidate, eg, "int" and "Long"
					// can
					// never both accomodate the same parameter; while "long"
					// and
					// "Integer" can.
			(toClass == Short.TYPE) || (toClass == Integer.TYPE) || (toClass == Long.TYPE) || (toClass == Float.TYPE)
					|| (toClass == Double.TYPE))
				return true;
		} else if ((fromClass == Character.TYPE) || fromName.equals(nonPrimTypeNames[2])) {
			if ((toClass == Character.TYPE) || toName.equals(nonPrimTypeNames[2]) || (toClass == Integer.TYPE)
					|| (toClass == Long.TYPE) || (toClass == Float.TYPE) || (toClass == Double.TYPE))
				return true;
	 ...
	 *
	 * @param clazz
	 *            The String that contains the JVM type name
	 *
	...
	/**
	 * Given an inheritance chain of types, stretching from a superclass down to
	 * a terminal concrete class, construct a map of generic types to their
	 * resolved types.
	 */
	...
	/**
	 * Given a generic type, add its parameter types to an evolving map of
	 * resolved types. Some of the resolved types may be generic type variables
	 * which will need further resolution from other generic types.
	 */
	...
	/**
	 * Given a map of resolved types, compose them together in order to resolve
	 * the actual concrete types that are plugged into the parameterized type.
	 */
