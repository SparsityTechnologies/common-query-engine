...
public class BrokeredConnection implements EngineConnection {
	...
	/**
	 * Maintain state as seen by this Connection handle, not the state of the
	 * underlying Connection it is attached to.
	 */
	...
	public final PreparedStatement prepareStatement(String sql)
		...
		try {
			return control.wrapStatement(
					getRealConnection().prepareStatement(sql), sql, null);
		} catch (SQLException sqle) {
			notifyException(sqle);
			throw sqle;
		}
	...
	public final CallableStatement prepareCall(String sql) throws SQLException {
		try {
			return control.wrapStatement(getRealConnection().prepareCall(sql),
					sql);
		} catch (SQLException sqle) {
			notifyException(sqle);
			throw sqle;
	...
	public final void setTypeMap(java.util.Map<String, Class<?>> map)
			throws SQLException {
		try {
	...
	public final PreparedStatement prepareStatement(String sql,
		...
		try {
			return control.wrapStatement(
					getRealConnection().prepareStatement(sql, columnIndexes),
					sql, columnIndexes);
		} catch (SQLException se) {
			notifyException(se);
			throw se;
	...
	public final PreparedStatement prepareStatement(String sql,
		...
		try {
			return control.wrapStatement(
					getRealConnection().prepareStatement(sql, columnNames),
					sql, columnNames);
		} catch (SQLException se) {
			notifyException(se);
			throw se;
	...
	/**
	 * Sync up the state of the underlying connection with the state of this new
	 * handle.
	 */
	...
	/**
	 * Isolation level state in BrokeredConnection can get out of sync if the
	 * isolation is set using SQL rather than JDBC. In order to ensure correct
	 * state level information, this method is called at the start and end of a
	 * global transaction.
	 */
	...
	/**
	 * Set the state of the underlying connection according to the state of this
	 * connection's view of state.
	 * 
	 * @param complete
	 *            If true set the complete state of the underlying Connection,
	 *            otherwise set only the Connection related state (ie. the
	 *            non-transaction specific state).
	 */
	...
	public BrokeredPreparedStatement newBrokeredStatement(
		...
		try {
			return new BrokeredPreparedStatement(statementControl, sql,
					generatedKeys);
		} catch (SQLException sqle) {
			notifyException(sqle);
			throw sqle;
	...
	/**
	 * set the DrdaId for this connection. The drdaID prints with the statement
	 * text to the errror log
	 * 
	 * @param drdaID
	 ...
	 * 
	 * @throws SQLException
	 * @return current prepare isolation level See
	 *         EmbedConnection#getPrepareIsolation
	 */
	...
	/**
	 * Get the string representation for this connection. Return the class
	 * name/hash code and various debug information.
	 * 
	...
	/**
	 * Get the holdability for statements created by this connection when
	 * holdability is not passed in.
	 */
	 ...
	 * @throws SQLException
	 *             if an object that implements the {@code Blob} interface can
	 *             not be constructed, this method is called on a closed
	 ...
	 * @throws SQLException
	 *             if an object that implements the {@code Clob} interface can
	 *             not be constructed, this method is called on a closed
	 *             connection or a database access error occurs.
	 *
	 */
	 ...
	 *
	 * @param name
	 *            the property key {@code String}
	 * @param value
	 *            the property value {@code String}
	 * @exception SQLClientInfoException
	 *                if the property is not supported or the real connection
	...
	public final void setClientInfo(String name, String value)
		...
		try {
			...
			throw new SQLClientInfoException(se.getMessage(), se.getSQLState(),
					se.getErrorCode(),
					new FailedProperties40(FailedProperties40.makeProperties(
	 ...
	 *
	 * @param properties
	 *            a {@code Properties} object with the properties to set.
	 * @exception SQLClientInfoException
	 *                if the properties are not supported or the real connection
	 *                could not be obtained.
	 */
	 ...
	 *
	 * @param name
	 *            a {@code String} that is the property key to get.
		...
		try {
			...
			try {
				...
				throw ExceptionFactory.getInstance().getSQLException(
						SQLState.UNABLE_TO_UNWRAP, (SQLException) null,
						(Throwable) null, iface);
			}
		} catch (SQLException sqle) {
			notifyException(sqle);
			throw sqle;
	 ...
	 * Section 16.1.3.1 of JDBC 4.0 (proposed final draft) says the driver may
	 * change the holdabilty and add a SQLWarning to the Connection object.
	 * 
	 * This work-in-progress implementation throws an exception to match the old
	 * behaviour just as part of incremental development.
	 */
	final int statementHoldabilityCheck(int resultSetHoldability)
		...
		if (holdability != resultSetHoldability) {
			SQLWarning w = SQLWarningFactory
					.newSQLWarning(SQLState.HOLDABLE_RESULT_SET_NOT_AVAILABLE);
	...
	public void setNetworkTimeout(Executor executor, int milliseconds)
			throws SQLException {
		try {
			((EngineConnection) getRealConnection()).setNetworkTimeout(
