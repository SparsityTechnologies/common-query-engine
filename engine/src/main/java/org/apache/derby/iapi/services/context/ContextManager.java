...
public class ContextManager {
	...
	public final List<Context> getContextStack(String contextId) {
		...
		return cs == null ? Collections.<Context> emptyList() : cs.getUnmodifiableList();
	}
		...
		forever: for (;;) {
			...
			cleanup: for (int index = holder.size() - 1; index >= 0; index--) {
				...
				try {
					...
					if (reportError && diagActive && (errorSeverity >= extDiagSeverityLevel)) {
					...
					if (reportError(se)) {
						errorStringBuilder
								.appendln("Less severe exception raised during cleanup (ignored) " + se.getMessage());
					...
					if (reportError) {
						errorStringBuilder
								.appendln("Equally severe exception raised during cleanup (ignored) " + t.getMessage());
	...
	private boolean reportError(Throwable t) {
		...
		if (t instanceof StandardException) {
			...
			case StandardException.REPORT_DEFAULT:
				...
				return (level >= logSeverityLevel) || (level == ExceptionSeverity.NO_APPLICABLE_SEVERITY);
	...
	ContextManager(ContextService csf, HeaderPrintWriter stream) {
		...
		extDiagSeverityLevel = PropertyUtil.getSystemInt(Property.EXT_DIAG_SEVERITY_LEVEL,
				ExceptionSeverity.SESSION_SEVERITY);
	}
