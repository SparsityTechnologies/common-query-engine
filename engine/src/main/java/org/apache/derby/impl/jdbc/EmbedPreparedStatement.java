 ...
 */
public class EmbedPreparedStatement extends EmbedStatement implements
		EnginePreparedStatement {

	// Moving jdbc2.0 batch related code in this class because callableStatement
	// in jdbc 20 needs
	// this code too and it doesn't derive from prepared statement in jdbc 20 in
	// our implementation.

	protected ResultSetMetaData rMetaData;
	// bug 4579-If the prepared statement was revalidated after doing
	// getMetaData(), we
	// should get the metadata info again on next getMetaData(). We store the
	// generated
	// class name in following variable during getMetaData() call. If it differs
	// from the
	// current generated class name, then that indicates a refetch of metadata
	// is required.
	 ...
	 * <p>
	 * This is lazily set if we need it, and there are currently only two valid
	 * header formats to choose between.
	 * 
	...
	public EmbedPreparedStatement(EmbedConnection conn, String sql,
			...
			int[] columnIndexes, String[] columnNames, boolean prepare)
			throws SQLException {
		...
		if (prepare) {
			...
			try {
				...
				// bug 4838 - save the auto-generated key information in
				// activation. keeping this
				// information in lcc will not work work as it can be tampered
				// by a nested trasaction
	...
	public EmbedPreparedStatement(EmbedConnection conn, String sql,
		...
		this(conn, sql, forMetaData, resultSetType, resultSetConcurrency,
	...
	protected void finalize() throws Throwable {
		...
		/*
		 * * We mark the activation as not being used and* that is it. We rely
		 * on the connection to sweep* through the activations to find the ones
		 * that* aren't in use, and to close them. We cannot* do a
		 * activation.close() here because there are* synchronized methods under
		 * close that cannot* be called during finalization.
		 */
		if (activation != null) {
	...
	/*
	 * Statement interface we override all Statement methods that take a SQL
	 * string as they must thrown an exception in a PreparedStatement. See the
	 * JDBC 3.0 spec.
	 */
	...
	public final boolean execute(String sql, int autoGenKeys)
			throws SQLException {
		throw newSQLException(SQLState.NOT_FOR_PREPARED_STATEMENT,
				"execute(String, int)");
	}
	...
	public final int executeUpdate(String sql, int autoGenKeys)
			throws SQLException {
		throw newSQLException(SQLState.NOT_FOR_PREPARED_STATEMENT,
	...
	/**
	 * Additional close to close our activation. In the case that a XAConnection
	 * is involved in the creation of this PreparedStatement for e.g in the
	 * following case
	 *
	 * <code>
	 ...
	 * BrokeredPreparedStatement object. Hence when we call bcc.onStatementClose
	 * and pass the PreparedStatement that caused it applicationStatement will
	 * be the appropriate choice since it will contain the appropriate instance
	 * of PreparedStatement in each case
	 *
	 * @throws SQLException
	 *             upon failure
	 *
	...
	void closeActions() throws SQLException {
		...
		if (bcc != null) {
			java.sql.PreparedStatement ps_app = (java.sql.PreparedStatement) applicationStatement;
	...
	/*
	 * PreparedStatement interface; we have inherited from EmbedStatement to get
	 * the Statement interface for EmbedPreparedStatement (needed by
	 * PreparedStatement) These are the JDBC interface comments, so we know what
	 * to do.
	 */
	...
	public final java.sql.ResultSet executeQuery() throws SQLException {
		...
		if (SanityManager.DEBUG) {
			if (results == null)
				SanityManager
						.THROWASSERT("no results returned on executeQuery()");
		}
	 ...
	 * Execute a SQL INSERT, UPDATE or DELETE statement. In addition, SQL
	 * statements that return nothing such as SQL DDL statements can be
	 * executed. For use with statements which may touch more than
	...
	/**
	 * We do this inefficiently and read it all in here. The target type is
	 ...
	 * may be more practical to send it via a java.io.Reader. JDBC will read the
	 ...
	 *
	 * If <code>lengthLess</code> is <code>true</code>, the following conditions
	 * are either not checked or verified at the execution time of the prepared
	 * statement:
	 * <ol>
	 * <li>If the stream length is negative.
	 * <li>If the stream's actual length equals the specified length.
	 * </ol>
	 ...
	 *
	 * @param parameterIndex
	 *            the 1-based index of the parameter to set.
	 * @param reader
	 *            the data.
	 * @param lengthLess
	 *            tells whether we know the length of the data or not.
	 * @param length
	 *            the length of the data. Ignored if <code>lengthLess</code> is
	 *            <code>true</code>.
	 */
	private void setCharacterStreamInternal(int parameterIndex, Reader reader,
			final boolean lengthLess, long length) throws SQLException {
		 ...
		 * stored in an integer This checking needs to be done because currently
		 * derby does not support Clob sizes greater than 2G-1
		 */
		...
		try {
			...
			final StringDataValue dvd = (StringDataValue) getParms()
					.getParameter(parameterIndex - 1);
			...
			getParms().getParameterForSet(parameterIndex - 1).setValue(utfIn,
					usableLength);
	...
	/**
	 * Determines which header format to use for CLOBs when writing them to the
	 * store.
	 *
	 * @return {@code true} if the pre Derby 10.5 header format is to be used,
	 *         {@code false} if the new header format can be used (10.5 or
	 *         newer)
	...
	private Boolean usePreTenFiveHdrFormat() throws StandardException {
		...
		if (usePreTenFiveHdrFormat == null) {
			usePreTenFiveHdrFormat = Boolean.valueOf(!lcc.getDataDictionary()
	...
	private void setBinaryStreamInternal(int parameterIndex, InputStream x,
		...
		if (!lengthLess && length > Integer.MAX_VALUE) {
			throw newSQLException(SQLState.LANG_OUTSIDE_RANGE_FOR_DATATYPE,
					getParameterMetaData().getParameterTypeName(parameterIndex));
		}
	 ...
	 *                if Derby does not support the type specified in the
	 *                {@code sqlType} parameter
	 */
	...
	boolean executeBatchElement(Object batchElement) throws SQLException,
			StandardException {
	...
	public java.sql.ResultSetMetaData getMetaData() throws SQLException {
		checkExecStatus();
		synchronized (getConnectionSynchronization()) {
			// reason for casting is getActivationClass is not available on
			// PreparedStatement
			...
			try {
				...
				// getMetaData call,
				// then gcDuringGetMetaData wouldn't match with current
	...
	/**
	 * The interface says that the type of the Object parameter must be
	 * compatible with the type of the targetSqlType. We check that, and if it
	 * flies, we expect the underlying engine to do the required conversion once
	 * we pass in the value using its type. So, an Integer converting to a CHAR
	 * is done via setInteger() support on the underlying CHAR type.
	 *
	 * <p>
	...
	public final void setObject(int parameterIndex, Object x,
		...
		/*
		 * If the parameter type is DECIMAL or NUMERIC, then we need to set the
		 * correct scale or set it to the default which is zero for setObject.
		 */
	...
	public final void setObject(int parameterIndex, Object x)
		...
		// overview, that passing a untyped null into setObject() is not
		// allowed.
		...
		if (x instanceof java.util.Calendar) {
			setTimestamp(parameterIndex, new Timestamp(((java.util.Calendar) x)
					.getTime().getTime()));
			return;
		}
	...
	public void setBlob(int i, Blob x) throws SQLException {
		...
		if (x == null)
			...
			setBinaryStreamInternal(i, x.getBinaryStream(), false, streamLength);
		}
	}
	...
	public void setClob(int i, Clob x) throws SQLException {
		...
		if (x == null)
			...
			// 1. max number of characters that can be inserted into a clob
			// column
			...
			setCharacterStreamInternal(i, x.getCharacterStream(), false,
					streamLength);
		}
	 ...
	 * 
	 * The caller of this method should be aware that the activation associated
	 * with a Statement can change and hence the ParameterValueSet returned by
	 * this call should not be hold onto. An example of this can be seen in
	 * EmbedCallableStatement.executeStatement where at the beginning of the
	 * method, we check the validity of the parameters. But we donot keep the
	 * parameters in a local variable to use later. The reason for this is that
	 * the next call in the method, super.executeStatement can recompile the
	 * statement and create a new activation if the statement plan has been
	 * invalidated. To account for this possibility,
	 * EmbedCallableStatement.executeStatement makes another call to get the
	 * ParameterValueSet before stuffing the output parameter value into the
	 * ParameterValueSet object.
	 *
	 *
	...
	/**
	 * Get the target JDBC type for a parameter. Will throw exceptions if the
	 * parameter index is out of range. The parameterIndex is 1-based.
	 * 
	 * @exception SQLException
	...
	protected int getParameterJDBCType(int parameterIndex) throws SQLException {
		try {
			DataTypeDescriptor dtd = preparedStatement
					.getParameterType(parameterIndex - 1);

			return (dtd == null) ? Types.OTHER : dtd.getTypeId()
					.getJDBCTypeId();
	...
	private void setScale(int parameterIndex, int scale) throws SQLException {
		...
		try {
			...
			((VariableSizeDataValue) value).setWidth(
					VariableSizeDataValue.IGNORE_PRECISION, scale, false);

			if (value.getLength() < origvaluelen) {
				activation.addWarning(StandardException.newWarning(
						SQLState.LANG_VALUE_TRUNCATED, value.getString()));
			}
	...
	public ParameterMetaData getParameterMetaData() throws SQLException {
		checkStatus();
		return new EmbedParameterSetMetaData(getParms(),
	...
	//
	protected EmbedResultSetMetaData newEmbedResultSetMetaData(
	...
	public void transferParameters(EmbedPreparedStatement newStatement)
		...
		try {
			newStatement.activation.setParameters(getParms(),
					preparedStatement.getParameterTypes());
	...
	protected boolean executeStatement(Activation a, boolean executeQuery,
	...
	final SQLException dataTypeConversion(int column, String sourceType)
		...
		return newSQLException(SQLState.LANG_DATA_TYPE_GET_MISMATCH,
				getParameterMetaData().getParameterTypeName(column), sourceType);
	}

	/**
	 * This method is used to initialize the BrokeredConnectionControl variable
	 * with its implementation. This method will be called in the
	 * BrokeredConnectionControl class
	 *
	 * @param control
	 *            used to call the onStatementClose and onStatementErrorOccurred
	 *            methods that have logic to raise StatementEvents for the close
	 *            and error events on the PreparedStatement
	 *
	 */
	...
	/**
	 * Method calls onStatementError occurred on the BrokeredConnectionControl
	 * class after checking the SQLState of the SQLException thrown.
	 *
	...
	private void checkStatementValidity(SQLException sqle) throws SQLException {
		/*
		 * Check if the exception has occurred because the connection associated
		 * with the PreparedStatement has been closed
		 */
		if (bcc != null && isClosed()) {
			// call the BrokeredConnectionControl interface method
			// onStatementErrorOccurred
			bcc.onStatementErrorOccurred(
					(java.sql.PreparedStatement) applicationStatement, sqle);
		}
		throw sqle;
	}
	 ...
	 * object. The data will be read from the stream as needed until end-of-file
	 * is reached. The JDBC driver will do any necessary conversion from UNICODE
	 * to the database char format.
	 *
	 * <em>Note:</em> This stream object can either be a standard Java stream
	 * object or your own subclass that implements the standard interface.
	 *
	 * Using this lengthless overload is not less effective than using one where
	 * the stream length is specified, but since there is no length specified,
	 * the exact length check will not be performed.
	 *
	 * @param parameterIndex
	 *            the first parameter is 1, the second is 2, ...
	 * @param reader
	 ...
	 *             closed PreparedStatementor if parameterIndex does not
	 ...
	 * method differs from the <code>setBinaryStream(int, InputStream)
	 ...
	 * @throws SQLException
	 *             if a database access error occurs, this method is called on a
	 *             closed <code>PreparedStatement</code> or if
	 *             <code>parameterIndex</code> does not correspond to a
	 ...
	 *             zero or if the number of bytes in the inputstream does not
	 *             match the specfied length.
	 */
