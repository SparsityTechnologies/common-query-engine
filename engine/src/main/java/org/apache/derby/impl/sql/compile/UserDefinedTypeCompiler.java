...
class UserDefinedTypeCompiler extends BaseTypeCompiler {
	...
	public boolean convertible(TypeId otherType, boolean forDataTypeFunction) {
		...
		/*
		 ** We are a non-ANSI user defined type, we are going to have to let the
		 * client find out the hard way.
		 */
		return true;
	}
	...
	/**
	 * ANSI UDTs can only be stored into values of exactly their own type. This
	 * restriction can be lifted when we implement the ANSI subclassing clauses.
	 *
	 * Old-style User types are storable into other user types that they are
	 * assignable to. The other type must be a subclass of this type, or
	 * implement this type as one of its interfaces.
	 *
	 * @param otherType
	 *            the type of the instance to store into this type.
	 * @param cf
	 *            A ClassFactory
	...
	public boolean storable(TypeId otherType, ClassFactory cf) {
		...
		if (getTypeId().getCorrespondingJavaTypeName().equals(otherType.getCorrespondingJavaTypeName())) {
			return true;
		}
		...
		return cf.getClassInspector().assignableTo(otherType.getCorrespondingJavaTypeName(),
