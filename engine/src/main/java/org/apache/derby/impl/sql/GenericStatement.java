...
public class GenericStatement implements Statement {
	...
	public void setPrepareIsolationLevel(int prepareIsolationLevel) {
	...
	private PreparedStatement prepMinion(LanguageConnectionContext lcc,
		...
		/*
		 * beginTimestamp only meaningful if beginTime is meaningful. beginTime
		 * is meaningful if STATISTICS TIMING is ON.
		 */
		if (beginTime != 0) {
		 ...
		 * prepared statement when prepare was called. Now the functionality of
		 * CS has been folded into GenericStatement (GS). So we search the cache
		 * for an existing PreparedStatement only when cacheMe is TRUE. i.e if
		 * the user calls prepare with cacheMe set to TRUE: then we a) look for
		 * the prepared statement in the cache. b) add the prepared statement to
		 * the cache.
		 * 
			...
			for (;;) {
				...
				if (foundInCache) {
					if (preparedStmt.referencesSessionSchema()) {
						// cannot use this state since it is private to a
						// connection.
		...
		try {
			...
			/*
			 * * For stored prepared statements, we want all* errors, etc in the
			 * context of the underlying* EXECUTE STATEMENT statement, so don't
			 * push/pop* another statement context unless we don't have* one. We
			 * won't have one if it is an internal* SPS (e.g. jdbcmetadata).
			 */
			if (!preparedStmt.isStorable() || lcc.getStatementDepth() == 0) {
				...
				statementContext = lcc.pushStatementContext(true,
						isForReadOnly, getSource(), null, false, 0L);
			}

			/*
			 * * RESOLVE: we may ultimately wish to pass in* whether we are a
			 * jdbc metadata query or not to* get the CompilerContext to make
			 * the createDependency()* call a noop.
			 */
			...
			if (internalSQL || (spsSchema != null)
					&& (spsSchema.isSystemSchema())
			...
			try {
				...
				if (istream != null) {
					String xactId = lcc.getTransactionExecute()
							.getActiveStateTxIdString();
					istream.printlnWithHeader(LanguageConnectionContext.xidStr
							+ xactId + "), " + LanguageConnectionContext.lccStr
							...
							+ getSource() + " :End prepared statement");
				}
				...
				StatementNode qt = (StatementNode) p.parseStatement(
						statementText, paramDefaults);
				...
				if (SanityManager.DEBUG) {
					...
					if (SanityManager.DEBUG_ON("StopAfterParsing")) {
						...
						throw StandardException
								.newException(SQLState.LANG_STOP_AFTER_PARSING);
					}
				}

				/*
				 * * Tell the data dictionary that we are about to do* a bunch
				 * of "get" operations that must be consistent with* each other.
				 */
				...
				try {
					...
					if (SanityManager.DEBUG) {
						...
						if (SanityManager.DEBUG_ON("StopAfterBinding")) {
							throw StandardException
									.newException(SQLState.LANG_STOP_AFTER_BINDING);
						}
					}

					// Derby424 - In order to avoid caching select statements
					// referencing
					// any SESSION schema objects (including statements
					// referencing views
					// in SESSION schema), we need to do the SESSION schema
					// object check
					// here.
					// a specific eg for statement referencing a view in SESSION
					// schema
					...
					// and we
					// should not cache this statement because a user can later
					// define
					// a global temporary table with the same name as the view
					// name.
					// Following demonstrates that
					// DECLARE GLOBAL TEMPORARY TABLE SESSION.t28v1(c21 int, c22
					// int) not
					// logged
					// INSERT INTO SESSION.t28v1 VALUES (280,1),(281,2)
					// SELECT * from SESSION.t28v1 should show contents of
					// global temporary
					// table and not the view. Since this select statement was
					// not cached
					// earlier, it will be compiled again and will go to global
					// temporary
					// table to fetch data. This plan will not be cached either
					// because
					// select statement is using SESSION schema object.
					//
					// Following if statement makes sure that if the statement
					// is
					// referencing SESSION schema objects, then we do not want
					// to cache it.
					// We will remove the entry that was made into the cache for
					// this statement at the beginning of the compile phase.
					// The reason we do this check here rather than later in the
					// compile
					// phase is because for a view, later on, we loose the
					// information that
					// it was referencing SESSION schema because the reference
					// view gets replaced with the actual view definition. Right
					// after
					// binding, we still have the information on the view and
					// that is why
				...
				/*
				 * we need to move the commit of nested sub-transaction after we
				 * mark PS valid, during compilation, we might need to get some
				 * lock to synchronize with another thread's DDL execution, in
				 * particular, the compilation of insert/update/ delete vs.
				 * create index/constraint (see Beetle 3976). We can't release
				 * such lock until after we mark the PS valid. Otherwise we
				 * would just erase the DDL's invalidation when we mark it
				 * valid.
				 */
				try // put in try block, commit sub-transaction if bad
				{
					if (SanityManager.DEBUG) {
						...
						if (SanityManager.DEBUG_ON("StopAfterOptimizing")) {
							throw StandardException
									.newException(SQLState.LANG_STOP_AFTER_OPTIMIZING);
						}
					}

					GeneratedClass ac = qt.generate(preparedStmt
							.getByteCodeSaver());

					generateTime = getCurrentTimeMillis(lcc);
					/*
					 * endTimestamp only meaningful if generateTime is
					 * meaningful. generateTime is meaningful if STATISTICS
					 * TIMING is ON.
					 */
					if (generateTime != 0) {
					...
					if (SanityManager.DEBUG) {
						if (SanityManager.DEBUG_ON("StopAfterGenerating")) {
							throw StandardException
									.newException(SQLState.LANG_STOP_AFTER_GENERATING);
						}
					}

					/*
					 * copy over the compile-time created objects to the
					 * prepared statement. This always happens at the end of a
					 * compile, so there is no need to erase the previous
					 * entries on a re-compile -- this erases as it replaces.
					 * Set the activation class in case it came from a
					...
					preparedStmt.setRequiredPermissionsList(cc
					...
					preparedStmt.setExecuteStatementNameAndSchema(
							qt.executeStatementName(), qt.executeSchemaName());
				...
				if (lcc.getRunTimeStatisticsMode()) {
					preparedStmt.setCompileTimeMillis(parseTime - beginTime, // parse
																				// time
	...
	protected void walkAST(LanguageConnectionContext lcc, Visitable queryTree,
	...
	public void setPreparedStatement(GenericPreparedStatement preparedStmt) {
