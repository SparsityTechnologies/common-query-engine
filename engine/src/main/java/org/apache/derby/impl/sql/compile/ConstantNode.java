 ...
 * <p>
 * A NULL from the parser may not yet know its type; that must be set during
 * binding, as it is for parameters.
 * <p>
 * the DataValueDescriptor methods want to throw exceptions when they are of the
 * wrong type, but to do that they must check typeId when the value is null,
 * rather than the instanceof check they do for returning a valid value.
 * <p>
 * For code generation, we generate a static field. Then we set the field be the
 * proper constant expression (something like <code>
 * getDatavalueFactory().getCharDataValue("hello", ...)) </code>) in the
 * constructor of the generated method. Ideally we would have just
 */
abstract class ConstantNode extends ValueNode {
	DataValueDescriptor value;

	/*
	 * * In case generateExpression() is called twice (something* that probably
	 * wont happen but might), we will cache* our generated expression and just
	 * return a reference* to the field that holds our value (instead of having*
	 * two fields holding the same constant).
	 */

	ConstantNode(TypeId typeId, boolean nullable, int maximumWidth,
	 ...
	 * figuring out what the return type is for this expression. In this case,
	 * there are no sub-expressions, and the return type is already known, so
	 * this is just a stub.
	 *
	 * @param fromList
	 ...
	 *                Thrown on error. Although this class doesn't throw this
	 *                exception, it's subclasses do and hence this method
	...
	ValueNode bindExpression(FromList fromList, SubqueryList subqueryList,
		...
		/*
		 * * This has to be here for binding to work, but it doesn't* have to do
		 * anything, because the datatypes of constant nodes* are pre-generated
		 * by the parser.
		 */
		return this;
	}
	...
	/**
	 * For a ConstantNode, we generate the equivalent literal value. A null is
	 * generated as a Null value cast to the type of the constant node. The
	 * subtypes of ConstantNode generate literal expressions for non-null
	 * values.
	 *
	 * @param acb
	...
	void generateExpression(ExpressionClassBuilder acb, MethodBuilder mb)
		...
		if (dtd != null) {
			collationType = dtd.getCollationType();
		}
		/* Are we generating a SQL null value? */
		if (isNull()) {
			acb.generateNull(mb, getTypeCompiler(), collationType);
		} else {
			generateConstant(acb, mb); // ask sub type to give a constant,
										// usually a literal like 'hello'

			acb.generateDataValue(mb, getTypeCompiler(), collationType,
					(LocalField) null);
		}
	}
	...
	boolean isEquivalent(ValueNode o) throws StandardException {
		if (isSameNodeKind(o)) {
			...
			return ((other.getValue() == null && getValue() == null) || (other
					.getValue() != null && other.getValue().compare(getValue()) == 0));
		}
