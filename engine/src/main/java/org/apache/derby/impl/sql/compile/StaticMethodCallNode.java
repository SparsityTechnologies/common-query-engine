...
/**
 * A StaticMethodCallNode represents a static method call from a Class (as
 * opposed to from an Object).
 * 
 * For a procedure the call requires that the arguments be ? parameters. The
 * parameter is *logically* passed into the method call a number of different
 * ways.
 * 
 * <P>
 * For a application call like CALL MYPROC(?) the logically Java method call is
 * (in psuedo Java/SQL code) (examples with CHAR(10) parameter) <BR>
 * Fixed length IN parameters - com.acme.MyProcedureMethod(?) <BR>
 * Variable length IN parameters - com.acme.MyProcedureMethod(CAST (? AS
 * CHAR(10)) <BR>
 * Fixed length INOUT parameter - String[] holder = new String[] {?};
 * com.acme.MyProcedureMethod(holder); ? = holder[0] <BR>
 * Variable length INOUT parameter - String[] holder = new String[] {CAST (? AS
 * CHAR(10)}; com.acme.MyProcedureMethod(holder); ? = CAST (holder[0] AS
 * CHAR(10))
 * 
 * <BR>
 * Fixed length OUT parameter - String[] holder = new String[1];
 * com.acme.MyProcedureMethod(holder); ? = holder[0]
 * 
 * <BR>
 * Variable length INOUT parameter - String[] holder = new String[1];
 * com.acme.MyProcedureMethod(holder); ? = CAST (holder[0] AS CHAR(10))
 * 
 * 
 * <P>
 * For static method calls there is no pre-definition of an IN or INOUT
 * parameter, so a call to CallableStatement.registerOutParameter() makes the
 * parameter an INOUT parameter, provided: - the parameter is passed directly to
 * the method call (no casts or expressions). - the method's parameter type is a
 * Java array type.
 * 
 * Since this is a dynamic decision we compile in code to take both paths, based
 * upon a boolean is INOUT which is derived from the ParameterValueSet. Code is
 * logically (only single parameter String[] shown here). Note, no casts can
 * exist here.
 * 
 * boolean isINOUT = getParameterValueSet().getParameterMode(0) ==
 * PARAMETER_IN_OUT; if (isINOUT) { String[] holder = new String[] {?};
 * com.acme.MyProcedureMethod(holder); ? = holder[0]
 * 
 * } else { com.acme.MyProcedureMethod(?) }
 * 
 *
 */
public class StaticMethodCallNode extends MethodCallNode {
	...
	/**
	 * Generated boolean field to hold the indicator for if any of the
	 * parameters to a RETURNS NULL ON NULL INPUT function are NULL. Only set if
	 * this node is calling such a function. Set at generation time.
	 */
	 ...
	 * @param javaClassName
	 *            The name of the java class that the static method belongs to.
	 * @param cm
	 *            The context manager
	 */
	public StaticMethodCallNode(String methodName, String javaClassName,
			ContextManager cm) {
		super(methodName, cm);
	 ...
	 *            The context manager
	 */
	public StaticMethodCallNode(TableName methodName, String javaClassName,
			ContextManager cm) {
	 ...
	 *                Thrown on error
	 */
	JavaValueNode bindExpression(FromList fromList, SubqueryList subqueryList,
	...
	private JavaValueNode bindExpressionMinion(FromList fromList,
		 ...
		 * is an alias and we must go to sysmethods to get the real method and
		 * java class names for this alias.
		 */
		if (javaClassName == null) {
			...
			if ((ad != null)
					&& (ad.getAliasType() == AliasInfo.ALIAS_TYPE_AGGREGATE_AS_CHAR)) {
				resolvedAggregate = new AggregateNode(
						((SQLToJavaValueNode) methodParms[0]).getSQLValueNode(),
						new UserAggregateDefinition(ad), procedureName, false,
						ad.getJavaClassName(), getContextManager());

				// Propagate tags used to flag nodes which need privilege
				// checks. See DERBY-6429.
				resolvedAggregate.copyTagsFrom(this);

				// The parser may have noticed that this aggregate is invoked in
				// a
			...
			if (!routineInfo.isDeterministic()) {
				checkReliability(getMethodName(),
						CompilerContext.NON_DETERMINISTIC_ILLEGAL);
			}
			...
			if (javaClassName.startsWith("org.apache.derby.")
					&& !javaClassName
							.startsWith("org.apache.derby.impl.tools.optional.")
					&& !javaClassName
							.startsWith("org.apache.derby.optional.lucene.")
					&& !javaClassName
							.startsWith("org.apache.derby.optional.api.")
		...
		if (routineInfo != null) {
			...
			if (returnType != null && !returnType.isRowMultiSet()
					&& !returnType.isUserDefinedType()) {
				TypeId returnTypeId = TypeId.getBuiltInTypeId(returnType
						.getJDBCTypeId());
				...
				if (returnTypeId.variableLength()) {
					...
					DataTypeDescriptor returnValueDtd = new DataTypeDescriptor(
							returnTypeId, returnType.getPrecision(),
							returnType.getScale(), returnType.isNullable(),
							returnType.getMaximumWidth());

					ValueNode returnValueToSQL = new JavaToSQLValueNode(this,
							getContextManager());

					ValueNode returnValueCastNode = new CastNode(
							returnValueToSQL, returnValueDtd,
							getContextManager());
					...
					returnValueToJava.setCollationType(returnType
							.getCollationType());
					return returnValueToJava.bindExpression(fromList,
	 ...
	 * nodes over the java value node for such parameters. This is because SQL
	 * functions defined with CALLED ON NULL INPUT need access to only java
	 * domain values. This can't be done for parameters which are wrappers over
	 * SQL function defined with RETURN NULL ON NULL INPUT because such
	 * functions need access to both sql domain value and java domain value. -
	 * Derby479 This optimization is not available if the outer function is
	...
	private void optimizeDomainValueConversion() throws StandardException {
		...
		for (int parm = 0; parm < count; parm++) {
			//
			// We also skip the optimization if the argument must be cast to a
			// primitive. In this case we need
			// a runtime check to make sure that the argument is not null. See
			// DERBY-4459.
			//
			if ((methodParms != null)
					&& methodParms[parm].mustCastToPrimitive()) {
				continue;
			}

			if (methodParms[parm] instanceof SQLToJavaValueNode
					&& ((SQLToJavaValueNode) methodParms[parm])
							.getSQLValueNode() instanceof JavaToSQLValueNode) {
				// If we are here, then it means that the parameter is
				// SQLToJavaValueNode on top of JavaToSQLValueNode
				JavaValueNode paramIsJavaValueNode = ((JavaToSQLValueNode) ((SQLToJavaValueNode) methodParms[parm])
						.getSQLValueNode()).getJavaValueNode();
				if (paramIsJavaValueNode instanceof StaticMethodCallNode) {
					...
					if (paramIsMethodCallNode.routineInfo != null
							&& paramIsMethodCallNode.routineInfo
									.calledOnNullInput())
						methodParms[parm] = ((JavaToSQLValueNode) ((SQLToJavaValueNode) methodParms[parm])
								.getSQLValueNode()).getJavaValueNode();
				}
			}
		}
	...
	/**
	 * Resolve a routine. Obtain a list of routines from the data dictionary of
	 * the correct type (functions or procedures) and name. Pick the best
	 * routine from the list. Currently only a single routine with a given type
	 * and name is allowed, thus if changes are made to support overloaded
	 * routines, careful code inspection and testing will be required.
	 */
	private void resolveRoutine(FromList fromList, SubqueryList subqueryList,
		...
		if (sd.getUUID() != null) {
			...
			List<AliasDescriptor> list = getDataDictionary()
					.getRoutineList(
							sd.getUUID().toString(),
							methodName,
							forCallStatement ? AliasInfo.ALIAS_NAME_SPACE_PROCEDURE_AS_CHAR
			...
			for (int i = list.size() - 1; i >= 0; i--) {
				...
				// pre-form the method signature. If it is a dynamic result set
				// procedure
				...
				for (int p = 0; p < parameterCount; p++) {
					...
					int parameterMode = rai.getParameterModes()[getRoutineArgIdx(
							rai, p)];
					...
					if (parameterMode != (ParameterMetaData.parameterModeIn)) {
						...
						case java.sql.Types.DOUBLE:
							arrayType = getTypeCompiler(typeId)
					...
					// this is the type descriptor of the require method
					// parameter
					DataTypeDescriptor methoddtd = new DataTypeDescriptor(
							typeId, td.getPrecision(), td.getScale(),
							td.isNullable(), td.getMaximumWidth());
					...
					DataTypeDescriptor paramdtd = new DataTypeDescriptor(
							parameterTypeId, td.getPrecision(), td.getScale(),
							td.isNullable(), td.getMaximumWidth());

					//
					// Now coerce the actual method parameter to the declared
					// type
					// of this routine arg.
					//
					...
					if (hasVarargs && (p == parameterCount - 1)) {
						//
						// The invocation of a varargs routine may have more
						// actual parameters
						// than the number of declared routine arguments. All of
						// the trailing
						// parameters must be coercible to the type of the last
						// declared argument.
						// Furthermore, it may turn out that there isn't a
						// parameter corresponding to the last
						// declared argument of the varargs routine.
						//
						for (int idx = p; idx < methodParms.length; idx++) {
							coerceMethodParameter(fromList, subqueryList,
									aggregates, rai, methodParms.length,
					...
					{
						coerceMethodParameter(fromList, subqueryList,
								aggregates, rai, methodParms.length, paramdtd,
								parameterTypeId, parameterMode, p);
					}
				}
				...
				// If a procedure is in the system schema and defined as
				// executing
		...
		if ((ad == null) && (methodParms.length == 1)) {
			ad = AggregateNode.resolveAggregate(getDataDictionary(), sd,
					methodName);
		}
	}
	...
	/**
	 * <p>
	 * Coerce an actual method parameter to the declared type of the
	 * corresponding routine argument.
	 * </p>
	 */
	private void coerceMethodParameter(FromList fromList,
			SubqueryList subqueryList, List<AggregateNode> aggregates,
			RoutineAliasInfo rai, int parameterCount, // number of declared
														// routine args
			DataTypeDescriptor paramdtd, // declared type of routine arg
			TypeId parameterTypeId, // declared type id of routine arg
			int parameterMode, int p // index of actual method parameter in
										// array of parameters
	) throws StandardException {
		...
		if ((sqlParamNode == null) || !sqlParamNode.requiresTypeFromContext()) {
			...
			if (sqlParamNode instanceof UnaryOperatorNode) {
				ParameterNode pn = ((UnaryOperatorNode) sqlParamNode)
						.getParameterOperand();
				applicationParameterNumbers[p] = pn.getParameterNumber();
			} else {
				applicationParameterNumbers[p] = ((ParameterNode) sqlParamNode)
						.getParameterNumber();
			}
		}
		...
		if (!isParameterMarker) {
			...
			if (sqlParamNode instanceof UntypedNullConstantNode) {
				...
				if (sqlParamNode != null) {
					...
					dts = DataTypeDescriptor
							.getSQLDataTypeDescriptor(methodParms[p]
									.getJavaTypeName());
					if (dts == null) {
						throw StandardException.newException(
				...
				if (!getTypeCompiler(parameterTypeId).storable(argumentTypeId,
						getClassFactory())) {
					throw StandardException.newException(
				...
				// if it's not an exact length match then some cast will be
				// needed.
		...
		if (needCast) {
			...
			if (sqlParamNode == null) {
				sqlParamNode = new JavaToSQLValueNode(methodParms[p],
						getContextManager());
			}

			ValueNode castNode = makeCast(sqlParamNode, paramdtd,
					getContextManager());

			methodParms[p] = new SQLToJavaValueNode(castNode,
					getContextManager());

			methodParms[p] = methodParms[p].bindExpression(fromList,
					subqueryList, aggregates);
		}
	...
	public static ValueNode makeCast(ValueNode parameterNode,
			DataTypeDescriptor targetType, ContextManager cm)
			throws StandardException {
	...
	/**
	 * Add code to set up the SQL session context for a stored procedure or
	 * function which needs a nested SQL session context (only needed for those
	 * which can contain SQL).
	 *
	 ...
	 *
	 * @param acb
	 *            activation class builder
	 * @param mb
	 *            method builder
	 */
	private void generatePushNestedSessionContext(ActivationClassBuilder acb,
			MethodBuilder mb, boolean hadDefinersRights, String definer)
			throws StandardException {
		...
		mb.push(definer);
		mb.callMethod(VMOpcode.INVOKEINTERFACE, null,
				"pushNestedSessionContext", "void", 3);
	}

	/**
	 * Push extra code to generate the casts within the arrays for the
	 * parameters passed as arrays.
	 */
	@Override
	void generateOneParameter(ExpressionClassBuilder acb, MethodBuilder mb,
			int parameterNumber) throws StandardException {
		int parameterMode;
		...
		if (routineInfo != null) {
			...
			// for a static method call the parameter always starts out as a in
			// parameter, but
			// may be registered as an IN OUT parameter. For a static method
			// argument to be
			// a dynmaically registered out parameter it must be a simple ?
			// parameter
			...
			if (sql2j != null) {
				if (sql2j.getSQLValueNode().requiresTypeFromContext()) {
					...
					if (sql2j.getSQLValueNode() instanceof UnaryOperatorNode)
						pn = ((UnaryOperatorNode) sql2j.getSQLValueNode())
								.getParameterOperand();
					else
						pn = (ParameterNode) (sql2j.getSQLValueNode());

					// applicationParameterNumbers is only set up for a
					// procedure.
					...
					if (parameterType.endsWith("[]")) {
						...
						constructor
								.push(ParameterMetaData.parameterModeUnknown);
						constructor.callMethod(VMOpcode.INVOKEINTERFACE, null,
		...
		case (ParameterMetaData.parameterModeOut): {
			...
			String arrayType = methodParameterType.substring(0,
					methodParameterType.length() - 2);

			// if a varargs arg, then strip off the extra array dimension added
			// by varargs
	...
	boolean categorize(JBitSet referencedTabs, boolean simplePredsOnly)
		...
		pushable = pushable
				&& super.categorize(referencedTabs, simplePredsOnly);
	...
	void generateExpression(ExpressionClassBuilder acb, MethodBuilder mb)
		...
		if (routineInfo != null) {
			...
			if (!routineInfo.calledOnNullInput()
				...
				returnsNullOnNullState = acb.newFieldDeclaration(
		...
		if (routineInfo != null) {
			...
			// Before we set up our authorization level, add a check to see if
			// this
			// method can be called. If the routine is NO SQL or CONTAINS SQL
			// then there is no need for a check. As follows:
			//
			// Current Level = NO_SQL - CALL will be rejected when getting CALL
			// result set
			// Current Level = anything else - calls to procedures defined as
			// NO_SQL and CONTAINS SQL both allowed.
			...
			if (sqlAllowed != RoutineAliasInfo.NO_SQL) {
				...
				generateAuthorizeCheck((ActivationClassBuilder) acb, mb,
						sqlOperation);
			}
			...
			if (statmentContextReferences != 0) {
				...
				mb.callMethod(VMOpcode.INVOKEINTERFACE, null,
						"getStatementContext",
						"org.apache.derby.iapi.sql.conn.StatementContext", 0);
			...
			/**
			 * Set the statement context to reflect we are running System
			 * procedures, so that we can execute non-standard SQL.
			 */
			if (isSystemCode) {
			...
			// context.
			if (sqlAllowed != RoutineAliasInfo.NO_SQL) {
				generatePushNestedSessionContext((ActivationClassBuilder) acb,
						mb, routineInfo.hasDefinersRights(), routineDefiner);
			}
			...
			//
			if (isFunction) {
				functionEntrySQLAllowed = acb.newFieldDeclaration(
						Modifier.PRIVATE, "short");
			...
			mb.push(sqlAllowed);
			mb.push(false);
			mb.callMethod(VMOpcode.INVOKEINTERFACE, null, "setSQLAllowed",
					"void", 2);
		...
		if (routineInfo != null && !hasVarargs()) {
			int compiledResultSets = methodParameterTypes.length
					- methodParms.length;

			if (compiledResultSets != 0) {

				// Add a method that indicates the maxium number of dynamic
				// result sets.
				...
				// add a method to return all the dynamic result sets
				// (unordered)
				MethodBuilder gdr = acb.getClassBuilder().newMethodBuilder(
						Modifier.PUBLIC, "java.sql.ResultSet[][]",
						"getDynamicResults");
				...
				// if (procDef.getParameterStyle() == RoutineAliasInfo.PS_JAVA)
				{
					...
					LocalField procedureResultSetsHolder = acb
					...
					cons.pushNewArray("java.sql.ResultSet[]",
							compiledResultSets);
		...
		if (returnsNullOnNullState != null) {
			mbnc = acb.newGeneratedFun(javaReturnType, Modifier.PRIVATE,
					methodParameterTypes);

			// add the throws clause for the public static method we are going
			// to call.
			Class[] throwsSet = ((java.lang.reflect.Method) method)
					.getExceptionTypes();
		...
		mbcm.callMethod(VMOpcode.INVOKESTATIC, method.getDeclaringClass()
				.getName(), methodName, actualMethodReturnType, nargs);
		...
		if (returnsNullOnNullState != null) {
			...
			mb.callMethod(VMOpcode.INVOKEVIRTUAL, acb.getClassBuilder()
					.getFullName(), mbnc.getName(), javaReturnType, nargs);
			mbnc = null;
		}
		...
		if (routineInfo != null) {
			...
			if (functionEntrySQLAllowed != null) {
				...
				mb.push(true); // override as we are ending the control set by
								// this function all.
				mb.callMethod(VMOpcode.INVOKEINTERFACE, null, "setSQLAllowed",
						"void", 2);
			...
			if (outParamArrays != null) {
				...
				for (int i = 0; i < outParamArrays.length; i++) {
					...
					if (parameterMode != (ParameterMetaData.parameterModeIn)) {
						...
						// Set the correct parameter nodes in the
						// ParameterValueSet at constructor time.
						constructor.dup();
						constructor.push(applicationParameterNumber);
						...
						// Pass the value of the outparameters back to the
						// calling code
						...
						mb.callMethod(VMOpcode.INVOKEINTERFACE, null,
								"getParameter", ClassName.DataValueDescriptor,
								1);

						// see if we need to set the desired
						// length/scale/precision of the type
						DataTypeDescriptor paramdtd = sqlParamNode
								.getTypeServices();

						boolean isNumericType = paramdtd.getTypeId()
								.isNumericTypeId();
						boolean isAnsiUDT = paramdtd.getTypeId()
								.getBaseTypeId().isAnsiUDT();

						// is the underlying type for the OUT/INOUT parameter
						// primitive.
						// if this is a varargs arg then we have to strip off
						// another array level
						...
						if (isNumericType) {
							// need to up-cast as the setValue(Number) method
							// only exists on NumberDataValue

							if (!isPrimitive)
								mb.cast(ClassName.NumberDataValue);
						} else if (paramdtd.getTypeId().isBooleanTypeId()) {
							// need to cast as the setValue(Boolean) method only
							// exists on BooleanDataValue
							if (!isPrimitive)
								mb.cast(ClassName.BooleanDataValue);
						}

						if (paramdtd.getTypeId().variableLength()) {
							// need another DVD reference for the set width
							// below.
							mb.dup();
						}
						...
						// The value needs to be set thorugh the
						...
						if (paramdtd.getTypeId().variableLength()) {
							mb.push(isNumericType ? paramdtd.getPrecision()
									: paramdtd.getMaximumWidth());
							mb.push(paramdtd.getScale());
							mb.push(isNumericType);
							mb.callMethod(VMOpcode.INVOKEINTERFACE,
									ClassName.VariableSizeDataValue,
									"setWidth", "void", 3);
							// mb.endStatement();
						}
					}
