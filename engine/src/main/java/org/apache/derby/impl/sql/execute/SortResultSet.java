 ...
 * If distinct is true, removes all but one copy of duplicate rows using
 * DistinctAggregator, which really doesn't aggregate anything at all -- the
 * sorter assumes that the presence of an aggregator means that it should return
 * a single row for each set with identical ordering columns.
 * <p>
 * If aggregate is true, then it feeds any number of aggregates to the sorter.
 * Each aggregate is an instance of GenericAggregator which knows which
 * Aggregator to call to perform the aggregation.
 * <p>
 * Brief background on the sorter and aggregates: the sorter has some
 * rudimentary knowledge about aggregates. If it is passed aggregates, it will
 * eliminate duplicates on the ordering columns. In the process it will call the
 * aggregator on each row that is discarded.
 * <p>
 * Note that a DISTINCT on the SELECT list and an aggregate cannot be processed
 * by the same SortResultSet(), if there are both aggregates (distinct or
 * otherwise) and a DISTINCT on the select list, then 2 separate SortResultSets
 * are required (the DISTINCT is a sort on the output of the sort with the
 * aggregation).
 * <p>
 * Currently, all rows are fed through the sorter. This is true even if there is
 * no sorting needed. In this case we feed every row in and just pull every row
 * out (this is an obvious area for a performance improvement). We'll need to
 * know if the rows are sorted before we can make any optimizations in this
 * area.
 * <p>
 * <B>CLONING</B>: Cloning and sorts are an important topic. Currently we do a
 * lot of cloning. We clone the following:
 * <UL>
 * <LI>every row that is inserted into the sorter. We need to clone the rows
 * because the source result set might be reusing rows, and we need to be able
 * to accumulate the entire result set in the sorter.</LI>
 * <p>
 * There are two cloning APIs: cloning by the sorter on rows that are not
 * discarded as duplicates or cloning in the SortResultSet prior to inserting
 * into the sorter. If we have any aggregates at all we always clone prior to
 * inserting into the sorter. We need to do this because we have to set up the
 * aggregators before passing them into the sorter. When we don't have
 * aggregates we let the sorter to the cloning to avoid unnecessary clones on
 * duplicate rows that are going to be discarded anyway.
 *
 */
class SortResultSet extends NoPutResultSetImpl implements CursorResultSet {
	 ...
	 *            input result set
	 * @param distinct
	 *            if this is a DISTINCT select list. Also set to true for a
	 *            GROUP BY w/o aggretates
	 * @param isInSortedOrder
	 *            true if the source results are in sorted order
	 * @param orderingItem
	 *            indicates the number of the SavedObject off of the
	 *            PreparedStatement that holds the ColumOrdering array used by
	 *            this routine
	 * @param a
	 *            activation
	 * @param ra
	 *            saved object that generates an empty row
	 * @param maxRowSize
	 *            approx row size, passed to sorter
	 ...
	 *                Thrown on error
	 */
	public SortResultSet(NoPutResultSet s, boolean distinct,
			boolean isInSortedOrder, int orderingItem, Activation a, int ra,
			int maxRowSize, int resultSetNumber,
			double optimizerEstimatedRowCount, double optimizerEstimatedCost)
			throws StandardException {
		super(a, resultSetNumber, optimizerEstimatedRowCount,
		...
		sortTemplateRow = ((ExecRowBuilder) ps.getSavedObject(ra)).build(a
				.getExecutionFactory());
		...
		/*
		 * NOTE: We need to save order to another variable in the constructor
		 * and reset it on every open. This is important because order can get
		 * reset in the guts of execution below. Subsequent sorts could get the
		 * wrong result without this logic.
		 */
		savedOrder = order;

		/*
		 * * Create a sort observer that are retained by the* sort.
		 */
	...
	public void openCore() throws StandardException {
		...
		try {
			 ...
			 * (We filter out the duplicate rows ourselves.) We save a clone of
			 * the first row so that subsequent next()s do not overwrite the
			 * saved row.
			 */
	...
	/**
	 * Load up the sorter. Feed it every row from the source scan. When done,
	 * close the source scan and open the sort. Return the sort scan controller.
	 *
	 * @exception StandardException
	 *                thrown on failure.
	 *
	...
	private ScanController loadSorter() throws StandardException {
		...
		sortId = tc.createSort((Properties) null,
				sortTemplateRow.getRowArray(), order, observer, inOrder,
	 ...
	 *                thrown on failure.
	 * @exception StandardException
	 *                ResultSetNotOpen thrown if not yet open.
	 *
	...
	public ExecRow getNextRowCore() throws StandardException {
		...
		if (isInSortedOrder && distinct) {
			...
			/*
			 * If this is the 1st next, then simply return the 1st row (which we
			 * got on the open()).
			 */
			if (!nextCalled) {
				nextCalled = true;
			...
			while (sortResult != null) {
				/*
				 * We found a new row. Update the current row and return this
				 * one.
				 */
				if (!filterRow(currSortedRow, sortResult)) {
					/*
					 * Save a clone of the new row so that it doesn't get
					 * overwritten
					 */
			...
			ExecRow sortResult = getNextRowFromRS();
			if (distinct) {
				while (sortResult != null && currSortedRow != null
						&& filterRow(currSortedRow, sortResult)) {
					sortResult = getNextRowFromRS();
				}
			...
			if (sortResult != null) {
				rowsReturned++;
				numColumns = sortResult.getRowArray().length;
			}
			setCurrentRow(sortResult);
			if (sortResult != null && distinct) {
	 ...
	 * (This allows us to process in-order distincts without a sorter.)
	 *
	 * @param currRow
	 *            The current row.
	 * @param newRow
	...
	public void close() throws StandardException {
		...
		if (isOpen) {
			...
			SanityManager.DEBUG("CloseRepeatInfo",
					"Close of SortResultSet repeated");
	 ...
	 *            ENTIRE_RESULTSET_TREE - time spent in this ResultSet and
	 *            below.
	 *
	...
	/**
	 * This result set has its row location from the last fetch done. If the
	 * cursor is closed, a null is returned.
	 *
	 * @see CursorResultSet
	 *
	...
	public ExecRow getCurrentRow() throws StandardException {
		...
		/*
		 * DISTINCT assumes the currentRow is good, since it is the only one
		 * with access to its sort scan result
		 */
		return currentRow;
	}
	...
	private ExecRow getNextRowFromRS() throws StandardException {
		return (scanController == null) ? getRowFromResultSet()
				: getRowFromSorter();
	}
	...
	private void closeSource() throws StandardException {
		if (scanController == null) {
			/*
			 * * NOTE: do not null out source, we* may be opened again, in which
			 * case* we will open source again.
			 */
			source.close();
		} else {
