...
final class GenericParameterValueSet implements ParameterValueSet {
	 ...
	 *
	 * @param numParms
	 *            The number of parameters in the new ParameterValueSet
	 * @param hasReturnOutputParam
	 *            if we have a ? = call syntax. Note that this is NOT the same
	 *            thing as an output parameter -- return output parameters are
	 *            special cases of output parameters.
	 */
	GenericParameterValueSet(ClassInspector ci, int numParms,
		...
		for (int i = 0; i < numParms; i++) {
			/*
			 * * Last param is if this is a return output param. True if* we
			 * have an output param and we are on the 1st parameter.
			 */
			parms[i] = new GenericParameter(this,
					(hasReturnOutputParam && i == 0));
		}
	}
	...
	/**
	 * Initialize the set by allocating a holder DataValueDescriptor object for
	 * each parameter.
	 */
	public void initialize(DataTypeDescriptor[] types) throws StandardException {
		if (types != null) {
			for (int i = 0; i < parms.length; i++) {
				...
				parms[i].initialize(dtd.getNull(), dtd.getJDBCTypeId(), dtd
		...
		try {
			...
			if (gp.parameterMode == (ParameterMetaData.parameterModeOut))
				throw StandardException
						.newException(SQLState.LANG_RETURN_OUTPUT_PARAM_CANNOT_BE_SET);
		...
		try {
			...
			case (ParameterMetaData.parameterModeUnknown):
				throw StandardException.newException(
						SQLState.LANG_NOT_OUTPUT_PARAMETER,
	...
	public void setParameterAsObject(int position, Object value)
			throws StandardException {
		...
		if (value != null) {

			{
				...
				if (throwError) {
					throw StandardException.newException(
							SQLState.LANG_DATA_TYPE_SET_MISMATCH, t,
							ClassInspector.readableClassName(value.getClass()),
							gp.declaredClassName);
				}
			}
	...
	public void transferDataValues(ParameterValueSet pvstarget)
		...
		for (int i = firstParam; i < parms.length; i++) {
			...
			if (oldp.registerOutType != Types.NULL) {
				...
				pvstarget.registerOutParameter(i, oldp.registerOutType,
						oldp.registerOutScale);
	...
	public String toString() {
		/*
		 * This method needed for derby.language.logStatementText=true. Do not
		 * put under SanityManager.DEBUG.
		 */
	...
	/**
	 * Check the position number for a parameter and throw an exception if it is
	 * out of range.
	 *
	 * @param position
	 *            The position number to check
	 *
	 * @exception StandardException
	 *                Thrown if position number is out of range.
	 */
	private void checkPosition(int position) throws StandardException {
		if (position < 0 || position >= parms.length) {
			...
			if (parms.length == 0)
				throw StandardException
						.newException(SQLState.NO_INPUT_PARAMETERS);

			throw StandardException.newException(
					SQLState.LANG_INVALID_PARAM_POSITION,
					String.valueOf(position + 1), String.valueOf(parms.length));
		}
	}
	 ...
	 *            The ordinal parameterIndex of a parameter to set to the given
	 *            value.
	 * @param jdbcType
	 *            A type from java.sql.Types
	 * @param scale
	...
	/**
	 * Validate the parameters. This is done for situations where we cannot
	 * validate everything in the setXXX() calls. In particular, before we do an
	 * execute() on a CallableStatement, we need to go through the parameters
	 * and make sure that all parameters are set up properly. The motivator for
	 * this is that setXXX() can be called either before or after
	 * registerOutputParamter(), we cannot be sure we have the types correct
	 * until we get to execute().
	 *
	 * @exception StandardException
	...
	/**
	 * Return the parameter number (in jdbc lingo, i.e. 1 based) for the given
	 * parameter. Linear search.
	 *
	 ...
	 * If there are unknown parameter types they are forced to input types. i.e.
	 * Derby static method calls with parameters that are array.
	 * 
	 * @return true if a declared Java Procedure INOUT or OUT parameter is in
	...
	/**
	 * Is there a return output parameter in this pvs. A return parameter is
	 * from a CALL statement of the following syntax: ? = CALL myMethod()
	 *
	...
	public DataValueDescriptor getReturnValueForSet() throws StandardException {
		...
		if (SanityManager.DEBUG) {
			if (!hasReturnOutputParam)
				SanityManager
						.THROWASSERT("getReturnValueForSet called on non-return parameter");
		}
