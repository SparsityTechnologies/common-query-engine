...
public abstract class MethodCallNode extends JavaValueNode {
	...
	/**
	 * The name of the class containing the method. May not be known until
	 * bindExpression() has been called.
	 * 
	 * @see #bindExpression
	...
	/**
	 * True if this is an internal call, just used to set up a generated method
	 * call.
	 */
	boolean internalCall;

	/**
	 * For resolution of procedure INOUT/OUT parameters to the primitive form,
	 * such as int[]. May be null.
	 */
	...
	/**
	 * @return the name of the class that contains the method, null if not
	 *         known. It may not be known until this node has been bound.
	 */
	...
	/**
	 * @return get the Java method or constructor determined during the bind()
	 *         phase.
	 */
	...
	public void addParms(List<ValueNode> parameterList) throws StandardException {
		...
		for (int index = 0; index < plSize; index++) {
			 ...
			 */
			JavaValueNode jqt = new SQLToJavaValueNode(qt, getContextManager());
	...
	Class<?>[] getMethodParameterClasses() {
		...
		for (int i = 0; i < methodParms.length; i++) {
			...
			try {
				...
				/*
				 * We should never get this exception since we verified that the
				 * classes existed at bind time. Just return null.
				 */
	...
	public String toString() {
		if (SanityManager.DEBUG) {
			return "methodName: " + (methodName != null ? methodName : "null") + "\n" + super.toString();
		} else {
			return "";
		}
	}
	 ...
	 *                Thrown on error
	 */
	final void bindParameters(FromList fromList, SubqueryList subqueryList, List<AggregateNode> aggregates)
		...
		if (methodParms != null) {
			...
			for (int parm = 0; parm < count; parm++) {
				if (methodParms[parm] != null) {
					methodParms[parm] = methodParms[parm].bindExpression(fromList, subqueryList, aggregates);
	 ...
	 * QUERY_INVARIANT or CONSTANT. This is useful for VTIs - a VTI is a
	 * candidate for materialization if all of its parameters are
	...
	/**
	 * Build parameters for error message and throw the exception when there is
	 * no matching signature found.
	 *
	 * @param receiverTypeName
	 *            Type name for receiver
	 * @param parmTypeNames
	 *            Type names for parameters as object types
	 * @param primParmTypeNames
	 *            Type names for parameters as primitive types
	 *
	 * @exception StandardException
	 *                Thrown on error
	 */
	void throwNoMethodFound(String receiverTypeName, String[] parmTypeNames, String[] primParmTypeNames)
			throws StandardException {
		...
		for (int i = 0; i < paramCount; i++) {
			...
			if ((primParmTypeNames != null) && !primParmTypeNames[i].equals(parmTypeNames[i])) // has
																								// primitive
			{
	...
	void preprocess(int numTables, FromList outerFromList, SubqueryList outerSubqueryList,
		...
		if (methodParms != null) {
			for (parm = 0; parm < methodParms.length; parm++) {
				if (methodParms[parm] != null) {
					methodParms[parm].preprocess(numTables, outerFromList, outerSubqueryList, outerPredicateList);
				}
			}
		}
	}
		...
		if (methodParms != null) {
			for (param = 0; param < methodParms.length; param++) {
				if (methodParms[param] != null) {
					pushable = methodParms[param].categorize(referencedTabs, simplePredsOnly) && pushable;
				}
			}
		}

		/*
		 * We need to push down method call. Then the predicate can be used for
		 * start/stop key for index scan. The fact that method call's cost is
		 * not predictable and can be expensive doesn't mean we shouldn't push
		 * it down. Beetle 4826.
		 */
		return pushable;
	}
	...
	JavaValueNode remapColumnReferencesToExpressions() throws StandardException {
		...
		if (methodParms != null) {
			for (param = 0; param < methodParms.length; param++) {
				if (methodParms[param] != null) {
					methodParms[param] = methodParms[param].remapColumnReferencesToExpressions();
				}
			}
		}
		return this;
	...
	public int getFirstVarargIdx() {
		if(method instanceof Method){
			return ((Method) method).getParameterTypes().length - 1;
		}
		else if(method instanceof Constructor){
			return ((Constructor) method).getParameterTypes().length - 1;
		}
	...
	public int generateParameters(ExpressionClassBuilder acb, MethodBuilder mb) throws StandardException {
		...
		int nonVarargCount = hasVarargs() ? methodParameterTypes.length - 1 : methodParms.length;
		...
		if (hasVarargs()) {
			...
			if (this.methodParms.length == totalArgCount - 1) {
				JavaValueNode[] copy = new JavaValueNode[totalArgCount];
				for (int i = 0; i < copy.length - 1; i++) {
					copy[i] = methodParms[i];
				}
				UntypedNullConstantNode nullValue = new UntypedNullConstantNode(getContextManager());
				SQLToJavaValueNode javaNullValue = new SQLToJavaValueNode(nullValue, getContextManager());
				DataTypeDescriptor dts = DataTypeDescriptor
						.getSQLDataTypeDescriptor(methodParameterTypes[totalArgCount - 1]);
				javaNullValue.value.setType(dts);

				JSQLType[] copySig = new JSQLType[totalArgCount];
				for (int i = 0; i < copySig.length - 1; i++) {
					copySig[i] = signature[i];
				}
				copySig[totalArgCount - 1] = javaNullValue.getJSQLType();
				signature = copySig;
				methodParms = copy;
				String[] names = routineInfo.getParameterNames();
				String[] copyParamNames = new String[totalArgCount];
				for (int i = 0; i < names.length; i++) {
					copyParamNames[i] = names[i];
				}
				copyParamNames[totalArgCount - 1] = "";
				int[] modes = routineInfo.getParameterModes();
				int[] copyModes = new int[totalArgCount];
				for (int i = 0; i < modes.length; i++) {
					copyModes[i] = modes[i];
				}
				copyModes[totalArgCount - 1] = ParameterMetaData.parameterModeIn;

				TypeDescriptor[] copyTd = new TypeDescriptor[totalArgCount];
				TypeDescriptor[] td = routineInfo.getParameterTypes();

				for (int i = 0; i < td.length; i++) {
					copyTd[i] = td[i];
				}
				copyTd[totalArgCount - 1] = dts.getCatalogType();

				RoutineAliasInfo copyRoutineInfo = new RoutineAliasInfo(methodName, totalArgCount, copyParamNames,
						copyTd, copyModes, routineInfo.getMaxDynamicResultSets(), routineInfo.getParameterStyle(),
						routineInfo.getSQLAllowed(), routineInfo.isDeterministic(), true, false,
						routineInfo.calledOnNullInput(), routineInfo.getReturnType());

				routineInfo = copyRoutineInfo;
			}
			generateVarargs(acb, mb);
		}
	 ...
	 * </p>
	 */
	private void generateAndCastOneParameter(ExpressionClassBuilder acb, MethodBuilder mb, int param,
			String parameterType) throws StandardException {
		...
		if (!parameterType.equals(argumentType)) {
			//
			// This handles the conversion from primitive to wrapper type. See
			// DERBY-6511.
			// If the parameter type is the wrapper form of the primitive
			// argument type,
			// then call the "valueOf" static method of the wrapper type in
			// order to convert
			// the argument into a wrapper object. So, for instance, this
			// converts a primitive "int"
			// into a "java.lang.Integer".
			//
			if (ClassInspector.primitiveType(argumentType)
			...
			// to the parameter type.
				...
				/*
				 * * Set the parameter type in case the argument type is
				 * narrower* than the parameter type.
				 */
	...
	/**
	 * <p>
	 * Generate the trailing routine arguments into a varargs array and push
	 * that array onto the stack.
	 * </p>
	 */
	private void generateVarargs(ExpressionClassBuilder acb, MethodBuilder mb) throws StandardException {
		...
		// if we have to support vararg constructors in the future, then this
		// code
		...
		// if (!routineInfo.isTableFunction()) {
		// allocate an array to hold the varargs
		MethodBuilder cb = acb.getConstructor();
		boolean nullArray = (methodParms[firstVarargIdx] == null);
		if (nullArray) {
			varargCount = 0;
		}
		...
		for (int i = 0; i < varargCount; i++) {
			...
			mb.setArrayElement(i); // move the parameter into the array, pop
									// the
									// stack
		}

		// push the array onto the stack. it is the last parameter to the
		// varargs routine.
		mb.getField(arrayField);
	}
	...
	/**
	 * <p>
	 * Get the offset into the routine arguments corresponding to the index of
	 * the invocation parameter. The two indexes may be different in the case of
	 * varargs methods. There may be more invocation args than declared routine
	 * args. For a varargs routine, all of the trailing invocation parameters
	 * correspond to the last argument declared by the CREATE FUNCTION/PROCEDURE
	 * statement.
	 * </p>
	 */
	protected int getRoutineArgIdx(int invocationArgIdx) {
		if (routineInfo == null) {
			...
			return getRoutineArgIdx(routineInfo, invocationArgIdx);
		}
	}
	...
	static public String getParameterTypeName(JavaValueNode param) throws StandardException {
		String argumentType;
		...
		if (param.isPrimitiveType()) {
			argumentType = param.getPrimitiveTypeName();
		} else {
			argumentType = param.getJavaTypeName();
		}
	...
	/**
	 * Generate one parameter to the given method call. This method is overriden
	 * by RepStaticMethodCallNode.
	 *
	 * @param acb
	 *            The ExpressionClassBuilder for the class we're generating
	 * @param mb
	 ...
	 *
	 * @param parmTypeNames
	 *            String[] with the java type names for the parameters as
	...
	protected void resolveMethodCall(String javaClassName, boolean staticMethod) throws StandardException {
		...
		if (routineInfo == null && !internalCall) {
			...
			if ((getCompilerContext().getReliability() & CompilerContext.INTERNAL_SQL_ILLEGAL) != 0) {
				throw StandardException.newException(SQLState.LANG_SYNTAX_ERROR,
						javaClassName + (staticMethod ? "::" : ".") + methodName);
			}
		}*/
		...
		boolean hasDynamicResultSets = hasVarargs() ? false
		...
		/*
		 * the actual type of the trailing Java varargs arg is an array if
		 * (hasVarargs()) { parmTypeNames[count - 1] = parmTypeNames[count - 1]
		 * + "[]"; }
		 */

		try {
			method = classInspector.findPublicMethod(javaClassName, methodName, parmTypeNames, null, isParam,
					staticMethod, hasDynamicResultSets, hasVarargs());

			// DB2 LUW does not support Java object types for SMALLINT, INTEGER,
			// BIGINT, REAL, DOUBLE
			// and these are the only types that can map to a primitive or an
			// object type according
			// to SQL part 13. So we never have a second chance match.
			// Also if the DDL specified a signature, then no alternate
			// resolution
			if (signatureOffset == -1 && routineInfo == null) {
				...
				if (method == null) {
					...
					method = classInspector.findPublicMethod(javaClassName, methodName, parmTypeNames,
							primParmTypeNames, isParam, staticMethod, hasDynamicResultSets, hasVarargs());
				}
			}
		...
		for (int i = 0; i < methodParameterTypes.length && (routineInfo == null || i < routineInfo.getParameterCount()); i++) {
			...
			//
			// Strip off the array type if this is a varargs arg. We are only
			// interested in
			...
			if (ClassInspector.primitiveType(methodParameter)) {
				// varargs may be omitted, so there may not be an invocation
				// argument
		...
		/*
		 * bug 4450 - if the callable statement is ? = call form, generate the
		 * metadata infor for the return parameter. We don't really need that
		 * info in order to execute the callable statement. But with jdbc3.0,
		 * this information should be made available for return parameter
		 * through ParameterMetaData class. Parser sets a flag in
		 * compilercontext if ? = call. If the flag is set, we generate the
		 * metadata info for the return parameter and reset the flag in the
		 * compilercontext for future call statements
		 */
	...
	/**
	 * Parse the user supplied signature for a method and validate it, need to
	 * match the number of parameters passed in and match the valid types for
	 * the parameter.
	 * 
	 * @param offset
	 *            Character offset of first paren
	 * @param hasDynamicResultSets
	 *            Can ResultSet[] parameters be specified.
	 * @return The valid array of types for resolution.
	 * @throws StandardException
	 */
	private String[] parseValidateSignature(String externalName, int offset, boolean hasDynamicResultSets)
			throws StandardException {
		...
		for (count = 0; st.hasMoreTokens();) {
			...
			if (count > signature.length) {
				if (hasDynamicResultSets) {
					...
					String rsType = signature[signature.length - 1].getSQLType().getTypeId()
				...
				throw StandardException.newException(SQLState.SQLJ_SIGNATURE_PARAMETER_COUNT, Integer.toString(count),
			...
			throw StandardException.newException(SQLState.LANG_DATA_TYPE_GET_MISMATCH, type,
					paramTypeId.getSQLTypeName()); // type conversion error
		}
		...
		if (count < signatureTypes.length) {
			...
			throw StandardException.newException(SQLState.SQLJ_SIGNATURE_PARAMETER_COUNT, Integer.toString(count),
					Integer.toString(signature.length)); // too few
															// types
		}
	...
	/**
	 * Build an array of names of the argument types. These types are biased
	 * toward Java objects. That is, if an argument is of SQLType, then we map
	 * it to the corresponding Java synonym class (e.g., SQLINT is mapped to
	 * 'java.lang.Integer').
	 *
	 *
	 ...
	 * parameter is a ?.
	 *
	 * @return array of booleans denoting wheter or not a given method parameter
	 *         is a ?.
	 */
		...
		if (jsqlType != null) {
			...
			case JSQLType.SQLTYPE:
				...
				if (ctid == null) {
					...
					// DB2 LUW does not support Java object types for SMALLINT,
					// INTEGER, BIGINT, REAL, DOUBLE
					// and these are the only types that can map to a primitive
					// or an object type according
					// to SQL part 13. So always map to the primitive type. We
					// can not use the getPrimitiveSignature()
					// as it (incorrectly but historically always has) maps a
					// DECIMAL to a double.
		...
		for (int i = 0; i < count; i++) {
			...
			if (jsqlTyp == null) {
				...
				case JSQLType.SQLTYPE:
					...
					if ((procedurePrimitiveArrayType != null) && (i < procedurePrimitiveArrayType.length)
	 ...
	 * can be: VARIANT - variant within a scan (non-static field access)
	 * SCAN_INVARIANT - invariant within a scan (column references from outer
	 * tables) QUERY_INVARIANT - invariant within the life of a query (constant
	 * expressions)
	 *
	...
	int getOrderableVariantType() throws StandardException {
		...
		// params then query-invariant. This makes more sense, and we can
		// evaluate
		// only once per query (good for performance) because method call could
		// be
	...
	private int getVariantTypeOfParams() throws StandardException {
		...
		if (methodParms != null) {
			for (int parm = 0; parm < methodParms.length; parm++) {
				if (methodParms[parm] != null) {
					int paramVariantType = methodParms[parm].getOrderableVariantType();
					if (paramVariantType < variance) // return the most variant
														// type
