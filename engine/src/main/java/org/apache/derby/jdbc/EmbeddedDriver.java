   ...
   */
	...
	*/
	static void boot() {
        ...
        InputStream bootProperties = Thread.currentThread().getContextClassLoader().getResourceAsStream("boot.properties");
        ...
        if(bootProperties == null){
        	...
        	try {
				properties.load(bootProperties);
				
				String internalDriver = properties.getProperty("internal.driver");
				
				
				new JDBCBoot().boot(Attribute.PROTOCOL, pw, internalDriver);
				
			} catch (Exception e) {
				throw new RuntimeException("Error loading the boot.properties file", e);
			}
        }
	}
