...
/**
 * A tool that generates a list of required classes from a set of properties
 * files. The value of any property within a property file that starts with
 * 'derby.module.' is taken as a class name. That class name and all the clases
 * it requires are listed to System.out, to facilitate building a zip file.
 * Classes that start with 'java.' or 'javax.' are not listed and are not
 * checked for dependent classes.
 * <P>
 * If the class name starts with 'com.ibm.db2j.' then a messages.properties file
 * is searched for corresponding to that class, if one exists then is is added
 * to the list of files printed.
 * <P>
 * The search path for the classes is $CLASSPATH
 * <P>
 * If the system property cloudscapeOnly is set to true then only classes and
 * message.properties files are listed that start with com.ibm.db2j.
 * <P>
 * The output for each class or properties file is a relative file name that
 * uses '/' as the file separator. e.g.
 * 
 ...
 * 
 * 
 * Usage: java [-DignoreWebLogic=true] [-Dverbose=true] [-DcloudscapeOnly=true]
 * [-DruntimeOnly=true] [-Ddb2jtools=true] [-DportingOnly=true]
 * [-Doutputfile=<filename>] org.apache.derbyBuild.classlister property_file [
 * property_file ... ]
 **/
...
public class classlister {
	...
	protected Hashtable<String, Hashtable<String, String>> masterClassList = new Hashtable<String, Hashtable<String, String>>();
	...
	protected static final String[] propFiles = { "messages.properties",
			"instructions.properties", "metadata.properties" };
	...
	public classlister() {
		...
		keepRolling = true;//Boolean.getBoolean("keepRolling");
	...
	public void run() {
		...
		try {
			...
			pwOut = new PrintWriter(new BufferedWriter(new FileWriter(
					outFile.getPath()), 10000), true);
		...
		for (int i = 0; i < sets.length; i++) {
			...
			try {
				...
				String s = sets[i];
				System.out.println("Analyzing file: " + s);
				if (s.endsWith(".class")) {
		...
		for (Enumeration e = foundClasses.keys(); e.hasMoreElements();) {
			...
			System.out.println("proccessing " + name + " type " + type);
			...
			if (type.equals("class")) {
				...
				String aux = name.replace('.', '/') + ".class";
				System.out.println("adding into the output " + aux);
				pwOut.println(aux);
			} else {
				// is a file name
	...
	protected void findClasses(Properties pset) throws IOException {
		System.out.println("finding classes inside a property file");
	...
	protected void loadClasspath() {
		...
		for (int i = 0; i < classpath.length; i++) {
			...
			if (pathEntry.toUpperCase(java.util.Locale.ENGLISH)
					.endsWith(".ZIP")
					|| pathEntry.toUpperCase(java.util.Locale.ENGLISH)
							.endsWith(".JAR")) {
				ZipFile zipfile = null;
				try {
					zipfile = new ZipFile(pathEntry.replace('/',
							File.separatorChar));
				} catch (IOException ioe) {
		...
		for (int i = 0; i < classpath.length; i++) {
			...
			if (hash != null) {
				if (hash instanceof ZipFile)
					...
					File file = new File((File) hash, className.replace('/',
							File.separatorChar));
					if (beVerbose) {
	...
	protected void findDependencies(String className) throws IOException {
		...
		try {
			...
			if (!SanityManager.DEBUG
					&& className
							.matches("^org\\.apache\\.derby\\..*\\.sanity\\..*")) {
				// Don't include sanity classes in insane builds.
				System.out.println("Skipping sanity classes " + className);
				return;
			}
			...
			if (db2jtools || db2jdrda) {
				...
				if (className.startsWith("org.apache.derby.authentication.")
						...
						|| className
								.startsWith("org.apache.derby.iapi.sql.dictionary.")
			...
			if (db2jdrda) {
				...
				if (className.startsWith("org.apache.derby.impl.sql")
						|| className.startsWith("org.apache.derby.impl.jdbc")
						|| className
								.startsWith("org.apache.derby.impl.services")
						|| className.startsWith("org.apache.derby.iapi.")
						|| className.startsWith("org.apache.derby.security.")
						|| (className.startsWith("org.apache.derby.mbeans.") && !className
								.startsWith("org.apache.derby.mbeans.drda."))) {
					return;
				}
			}
			...
			/*
			 * org.apache.derby.iapi.reference.ClassName & RegisteredFormatIds
			 * has a list of all registered classes, If we pull this in then we
			 * will pull in the complete set of classes. So we add this to our
			 * list but don't dependency check it.
			 */
			...
			try {
				...
				System.out.println("adding into the foundClasses " + className);
				...
				if (is == null) {
					...
					//
					// Until there is a Java 8 whose stubs can be used to build
					// Derby, we will allow
					// the build to complete without the JDBC 4.2 support.
					//
					if (className.endsWith("Driver42")) {
						return;
					}

					pwOut.println("**error** Got NULL when looking for fileName = "
							+ fileName);
					if (!keepRolling) {
						System.exit(1);
					} else {
				...
				for (Enumeration e = ch
						/* .getClassInfo() */.referencedClasses(); e
						.hasMoreElements();) {
	...
	protected boolean isCloudscapeCode(String name) {
		return name.startsWith("com.ibm.db2j.")
				|| name.startsWith("com.ihost.cs.") || name.startsWith("db2j.")
	...
	protected void showAllItems() {
		...
		while (e.hasMoreElements()) {
			...
			pwOut.println(kid);
			Hashtable<String, Integer> scoreboard = new Hashtable<String, Integer>();
	...
	protected void unrollHashtable(String parent,
			Hashtable<String, String> current,
			Hashtable<String, Integer> scoreboard, int indentLevel) {
		String indentString = "  ";
		...
		while (e.hasMoreElements()) {
			...
			if (value != null) {
				...
				pwOut.println(key + "*****REPEATED class back at level "
						+ value + "****");
				return;
			}
			for (int i = 0; i < indentLevel; i++) {
			...
			Hashtable<String, String> currentsChildren = masterClassList
					.get(key);
			scoreboard.put(key, new Integer(indentLevel));
			unrollHashtable(key, currentsChildren, scoreboard,
					(indentLevel + 1));
			scoreboard.put(key, new Integer(indentLevel));
